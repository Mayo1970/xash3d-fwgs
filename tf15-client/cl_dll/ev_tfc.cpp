...
float EV_TFC_PlayTextureSound( int idx, pmtrace_t *ptr, float *vecSrc, float *vecEnd, int iBulletType )
{
	char chTextureType;
	float fvol, fvolbar;
	// const: only ever assigned string literals below, passed straight to
	// EV_PlaySound's const char* param -- same -Werror=write-strings fix
	// TFC-2 already applied to tfort.cpp/tforttm.cpp's string arrays.
	const char *rgsz[4];
	int cnt;
	float fattn;
	int entity;
...
// const: forwards straight to EV_TFC_DecalTrace's own const char* param
void EV_TFC_BenchmarkWallMark( pmtrace_t *pTrace, const char *name )
{
	EV_TFC_DecalTrace( pTrace, name );
}
