...
// TFC-3 defined these here as a stopgap while vgui_TeamFortressViewport.cpp
// was excluded (no VGUI1 yet). TFC-5 un-excludes it, and it's the real
// upstream owner of these five globals -- removed here to avoid a duplicate-
// definition link error now that both TUs would otherwise define them.

cvar_t *hud_textmode;
float g_hud_text_color[3];

// IVoiceStatusHelper only exists when USE_VGUI is on (voice_status.h, gated
// above); this whole helper only feeds GetClientVoiceMgr()->Init() below,
// itself also gated, so there's nothing for it to do with VGUI/voice off.
#if USE_VGUI
...
// Every one of these __CmdFunc_*/__MsgFunc_* handlers exists purely to
// forward into gViewPort -- kept as real functions (not #if'd out entirely)
// since they're registered by name elsewhere in this file (HOOK_COMMAND/
// HOOK_MESSAGE), just with their VGUI-dependent body gated the same way as
// everywhere else in this vendoring pass.
