...
int DLLEXPORT HUD_Key_Event( int down, int keynum, const char *pszCurrentBinding )
{
	...
	if ( gViewPort )
	{
		int ret = gViewPort->KeyInput( down, keynum, pszCurrentBinding );
		// TFC-5 diagnostic: confirms whether button presses actually reach
		// gViewPort at all, and what it decides to do with them (return 0 =
		// consumed here, engine's own VGui_KeyEvent never runs this frame;
		// return 1 = falls through to the engine). Only logs on press, not
		// release, to avoid doubling the line count. Remove once the MOTD/
		// team-select interaction bug is closed.
		if ( down )
			gEngfuncs.Con_Printf( "[tfcinput] HUD_Key_Event: key=%d ret=%d\n", keynum, ret );
		return ret;
	}
#endif
	return 1;
