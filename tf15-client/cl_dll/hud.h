...
//
//-----------------------------------------------------
// USE_VGUI-gated in hlsdk-portable's own hud.h; this SDK's copy was simply
// missing the guard (never defines USE_VGUI anywhere in the tree either).
// voice_status.h pulls in real VGUI1 headers (VGUI_Label.h etc, confirmed by
// a real "No such file or directory" build error) which aren't vendored in
// this project yet -- CHud itself doesn't reference CVoiceStatus at all, so
// this is safe to gate off entirely until VGUI lands (TFC-5).
#if USE_VGUI
