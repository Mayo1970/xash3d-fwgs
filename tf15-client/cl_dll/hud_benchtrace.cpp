...
// Unused dead include on every non-Windows platform (confirmed: zero
// dl*() calls anywhere in this file -- the real Trace_GetHopCount body
// below is #ifdef _WIN32-only, ICMP-based). No dlopen on PS3 either way.
#include <dlfcn.h>
#endif
...
int Trace_GetHopCount( char *pServer, int nMaxHops )
{
	...
	if ( !pfnICMPCreateFile || !pfnICMPCloseFile || !pfnICMPSendEcho )
	{
		return -1;
	}
