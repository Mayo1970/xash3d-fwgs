...
void CHudSpectator::DirectorMessage( int iSize, void *pbuf )
{
	...
	case DRC_CMD_CAMERA:
		if ( m_autoDirector->value )
		{
			...
			SetCameraView( vJumpOrigin, vJumpAngles, READ_BYTE() );
			m_ChaseEntity = READ_WORD();
		}
		break;
	...
	case DRC_CMD_BANNER:
		...
		gViewPort->UpdateSpectatorPanel();
#else
		READ_STRING(); // still consume the banner filename off the wire, even with no VGUI panel to show it in
#endif
		break;
	...
	case DRC_CMD_WAYPOINTS:
		...
		for ( int x = 0; x < i; x++ )
		{
			value = gHUD.m_flTime + (float)( READ_SHORT() ) / 100.0f;
			...
			AddWaypoint( value, vJumpOrigin, vJumpAngles, READ_BYTE(), READ_BYTE() );
		}
...
{
// Upstream's own if(!gViewPort) return; already made this whole function a
// no-op without a viewport (spectator mode is treated as VGUI-gated here,
// not just the ShowMenu() call below) -- wrapping the whole body reproduces
// that exact behavior at compile time instead of runtime.
#if USE_VGUI
