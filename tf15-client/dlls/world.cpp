...
CGameRules *InstallGameRules( const char *szGameName )
{
	if ( g_pGameRules )
	{
		delete g_pGameRules;
...
// Server_GetPhysicsInterface (and the gPhysicsInterface/g_physfuncs plumbing
// it needs) is excluded outright: it depends on "physint.h" via
// dlls/physcallback.h, and that header does not exist anywhere in the
// upstream tf15-client repo -- confirmed against a clean checkout, not a
// vendoring omission here. world.cpp doesn't even #include physcallback.h,
// so this was never reachable. The feature is genuinely optional (base
// hlsdk-portable's own world.cpp has no equivalent at all and works fine
// without it), and this entry point isn't in dlls/exports.txt, so the
// engine would never have called it even if it compiled. DispatchCreateEntity/
// DispatchPhysicsEntity above are kept -- they don't depend on the missing
// header and are harmless, unreferenced code, same as several other dead
// declarations found while vendoring this tree.
