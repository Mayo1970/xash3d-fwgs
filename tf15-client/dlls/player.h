...
public:
	...
	char m_szTeamName[TEAM_NAME_LENGTH]; // matches hlsdk-portable's own player.h exactly
	...
	// TFC-6 Phase 2b: napalm burn DoT, ticked from PlayerThink.
	float m_flTFBurnEnd;
	float m_flTFBurnNextTick;
	EHANDLE m_hTFEffectAttacker;
	...
	virtual int RemovePlayerItem( CBasePlayerItem *pItem, bool bCallHolster );
	...
	virtual int GiveAmmo( int iCount, const char *szName, int iMax, int *pIndex = NULL );
	...
	// Never called server-side; CBaseEntity's own EngineerUse (subs.cpp)
	// already returns FALSE by default, matched here since this override had
