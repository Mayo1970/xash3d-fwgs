...
void UTIL_PrecacheOtherWeapon( const char *szClassname )
{
	...
	{
		ItemInfo II = {0};
		// Spawn(), not just Precache(): the TFC weapons set clip-size / id
		// members (m_iMaxClipSize etc) in Spawn(), and GetItemInfo() reads
		// them. Precache-only left ItemInfoArray[].iMaxClip == 0, which made
		// AddPrimaryAmmo() force m_iClip = -1 (noclip) on every clip weapon.
		DispatchSpawn( pent );
...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	else if( ( m_pPlayer->pev->button & IN_ATTACK ) && CanAttack( m_flNextPrimaryAttack, gpGlobals->time, UseDecrement() ) )
	{
		...
		if( m_pPlayer->m_afButtonPressed & IN_ATTACK )
			g_engfuncs.pfnServerPrint( UTIL_VarArgs( "[tfc] fire %s clip=%d nextprim=%.2f\n",
				STRING( pev->classname ), m_iClip, m_flNextPrimaryAttack ) );

		PrimaryAttack();
	}
	else if( m_pPlayer->m_afButtonPressed & IN_ATTACK )
	{
		g_engfuncs.pfnServerPrint( UTIL_VarArgs( "[tfc] fire BLOCKED %s CanAttack=%d nextprim=%.2f m_flNextAttack=%.2f clip=%d\n",
			STRING( pev->classname ), CanAttack( m_flNextPrimaryAttack, gpGlobals->time, UseDecrement() ),
			m_flNextPrimaryAttack, m_pPlayer->m_flNextAttack, m_iClip ) );
	}
