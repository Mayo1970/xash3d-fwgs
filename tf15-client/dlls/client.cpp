...
void ClientCommand( edict_t *pEntity )
{
	...
	else if( ( ( pstr = strstr( pcmd, "weapon_" ) ) != NULL ) &&
	         ( pstr == pcmd || ( pstr == pcmd + 3 && !strncmp( pcmd, "tf_", 3 ) ) ) )
	{
		// TFC weapon-select commands are "tf_weapon_*", not "weapon_*" -- the
		// old `pstr == pcmd` check only matched the bare prefix, so switching
		// weapons on TFC printed "Unknown command: tf_weapon_ng".
...
int AddToFullPack( struct entity_state_s *state, int e, edict_t *ent, edict_t *host, int hostflags, int player, unsigned char *pSet )
{
	...
	if( player )
	{
		...
		// TFC-6 Phase 1: the client reads g_iTeamNumber / g_iPlayerClass
		// straight off entity_state_t (cl_dll/entity.cpp), so players need
		// both fields replicated -- not just non-players.
