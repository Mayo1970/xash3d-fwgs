...
public:
	...
	void KeyValuePartTwo( KeyValueData *pkvd );
	void KeyValuePartThree( KeyValueData *pkvd );
	...
	// TFC's teamplay_gamerules.cpp calls this polymorphically through a
	// CBaseEntity* (CHalfLifeTeamplay::GetTeamID), so it needs a real base
	// virtual here, matching hlsdk-portable's cbase.h -- missing in this tree.
	...
	virtual const char *DB_GetItemName( void ) { return ""; }
	...
	CBaseEntity *FindTimer( int iTimerType );
	CBaseEntity *CreateTimer( int iTimerType );
	BOOL CheckExistence( void );
	void CheckBelowBuilding( int iDist );
	int CheckArea( CBaseEntity *pIgnore );
	virtual void Finished( void ) { }

	BOOL IsAlly( CBaseEntity *pOther );
	BOOL IsAlly( int iTeamNo );
	BOOL IsTeammate( CBaseEntity *pOther );
	...
	// These four had no definition ANYWHERE in the server-only tree TFC-2
	// vendored (only TeamFortress_EMPRemove is overridden, by CItem in
	// items.cpp) -- real link errors, not guessed: every CBaseEntity-derived
	// class whose vtable doesn't override them (CTFNailgun, CTFRailgun,
	// CTFRpg, etc.) still needs the base slot resolvable. TFC-3 found the
	// real definitions do exist after all, in cl_dll/tfc/tf_baseentity.cpp
	// (a client-only stub file, Valve/Velaron's own precedent for exactly
	// this: the client doesn't compile dlls/*.cpp's real game logic, so it
	// stubs out every base virtual nothing else defines client-side) --
	// confirmed by a real "redefinition" error once that file entered the
	// build. CLIENT_DLL is this header's existing, established way to tell
	// client and server compiles apart (see weapons.h), so gate the body on
	// it: server keeps this inline no-op (unchanged from TFC-2, still the
	// only definition it has), client gets tf_baseentity.cpp's copy (also
	// inline no-ops here, so behavior is identical either way).
#ifndef CLIENT_DLL
	...
	void Timer_Infection( void );
	void Timer_HealthRot( void );
	void Timer_Hallucination( void );
	void Timer_Tranquilisation( void );
	void Timer_Regeneration( void );
	void Timer_Birthday( void );
	void Timer_DetpackSet( void );
	void Timer_DetpackDisarm( void );
	void Timer_FinishedBuilding( void );
	void Timer_CheckBuildDistance( void );
	void Timer_SpyUndercoverThink( void );
	void Timer_CeaseFireThink( void );
	void Timer_AutokickThink( void );

	void DoRespawn( void );

	void tfgoal_timer_tick( void );
	void tfgoalitem_dropthink( void );
	void tfgoalitem_droptouch( void );
	void tfgoalitem_remove( void );

	void ReturnItem( void );
	void DelayedResult( void );
	void EndRoundEnd( void );
	...
	BOOL is_detpacking;
	BOOL is_feigning;
	BOOL is_unableto_spy_or_teleport;
	BOOL has_disconnected;
	BOOL fClientGrenadePrimed;
	BOOL bRemoveGrenade;
	unsigned short m_usBuildingEvent;
	int m_iBuildingEventState;
	Vector m_vOldOrigin;
	int tfstate;
	int items;
	int weapon;
	int m_iPrimedGrenType;
	CBaseEntity *observer_list;
	EHANDLE m_pOtherSection;
	int m_iRightBound;
	int m_iLeftBound;
	float cl_no_grenades_1;
	float cl_no_grenades_2;
	int *current_ammo;
	float currentammo;
	int maxammo_medikit;
	int ammo_medikit;
	...
	float maxarmor;
	int exec_scripts;
	int exec_map_scripts;
	int display_class_briefing;
	int take_screenshots;
	int local_blood;
	float weaponmode;
	float motd;
	float current_menu;
	float menu_displaytime;
	float menu_refreshtime;
	float m_flSafetyCheckTime;
	int team_no;
	int lives;
	int infection_team_no;
	int real_frags;
	float respawn_time;
	float suicide_time;
	EHANDLE building;
	...
	float has_dispenser;
	float has_sentry;
	float has_entry_teleporter;
	float has_exit_teleporter;
	int weapons_carried;
	...
	int ex_skill_max;
	int increase_team[4];
	int increase_team_owned_by;
	float wait;
	int count;
	int axhitme;
	float drop_time;
	Vector redrop_origin;
	int redrop_count;
	...
	string_t org_broadcast;
	string_t org_team_broadcast;
	string_t org_non_team_broadcast;
	string_t org_owners_team_broadcast;
	string_t org_non_owners_team_broadcast;
	string_t org_team_drop;
	string_t org_non_team_drop;
	string_t org_message;
	string_t org_noise3;
	string_t org_noise4;
	string_t netname_broadcast;
