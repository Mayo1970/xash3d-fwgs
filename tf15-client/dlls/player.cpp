...
// TFC-6 Phase 1: was an empty stub, so AC wind-up / sniper zoom never slowed
// the player. Ports the client copy in cl_dll/tfc/tf_weapons.cpp verbatim.
void CBasePlayer::TeamFortress_SetSpeed( void )
{
	...
	else
	{
		...
		{
		case PC_SCOUT:    speed = PC_SCOUT_MAXSPEED;    break;
		case PC_SNIPER:   speed = PC_SNIPER_MAXSPEED;   break;
		case PC_SOLDIER:  speed = PC_SOLDIER_MAXSPEED;  break;
		case PC_DEMOMAN:  speed = PC_DEMOMAN_MAXSPEED;  break;
		case PC_MEDIC:    speed = PC_MEDIC_MAXSPEED;    break;
		case PC_HVYWEAP:  speed = PC_HVYWEAP_MAXSPEED;  break;
		case PC_PYRO:     speed = PC_PYRO_MAXSPEED;     break;
		case PC_ENGINEER: speed = PC_ENGINEER_MAXSPEED; break;
		case PC_SPY:      speed = PC_SPY_MAXSPEED;      break;
		case PC_CIVILIAN: speed = PC_CIVILIAN_MAXSPEED; break;
...
// Declared (player.h) and called from wpn_shared/tf_wpn_axe.cpp (melee kill
// bonus) but never defined anywhere in this tree. Routed through AddPoints
// rather than a bare pev->frags += iFrags, since AddPoints already sends the
// gmsgScoreInfo update the client's HUD/scoreboard needs -- a bare frags
// increment would be correct server-side but silently desync the display.
void CBasePlayer::TF_AddFrags( int iFrags )
{
	AddPoints( iFrags, TRUE );
}
		...
		if( m_rgpPlayerItems[i] )
		{
			...
			while( pPlayerItem )
			{
				...
				if( gun && gun->UseDecrement() )
				{
					...
					// TFC-6 Phase 1: the client decrements m_flNextReload every
					// frame (cl_dll/tfc/tf_weapons.cpp); the server never did, so
					// the tf_wpn_* shotgun/GL/RPG special-reload state machine
					// deadlocked at `m_flNextReload <= 0.0f` and the clip never
					// refilled on a listen server.
					gun->m_flNextReload = Q_max( gun->m_flNextReload - gpGlobals->frametime, -0.001f );
...
//
int CBasePlayer::GiveAmmo( int iCount, const char *szName, int iMax, int *pIndex )
{
	if( !szName )
	{
		// no ammo.
...
void CBasePlayer::ItemPostFrame()
{
...
#endif
	{
		if( ( m_afButtonPressed & IN_ATTACK ) )
			g_engfuncs.pfnServerPrint( UTIL_VarArgs( "[tfc] ItemPostFrame gated: m_flNextAttack=%.3f\n", m_flNextAttack ) );
		return;
	}
	...
	{
		if( ( m_afButtonPressed & IN_ATTACK ) )
			g_engfuncs.pfnServerPrint( "[tfc] ItemPostFrame: no active item\n" );
		return;
	}
...
#define clamp( val, min, max ) ( ((val) > (max)) ? (max) : ( ((val) < (min)) ? (min) : (val) ) )
	...
	if( (int)pev->armorvalue != m_iClientBattery )
	{
		...
		// gmsgBattery is registered as 4 bytes and the client's MsgFunc_Battery
		// reads two shorts (value, max). Sending only one made the engine drop
		// the whole message ("Battery expected 4 bytes, it written 2. Ignored"),
		// so the armor HUD never activated. TFC per-class max armor lives in
		// CBaseEntity::maxarmor.
		MESSAGE_BEGIN( MSG_ONE, gmsgBattery, NULL, pev );
			WRITE_SHORT( (int)pev->armorvalue );
			WRITE_SHORT( maxarmor > 0.0f ? (int)maxarmor : 100 );
		MESSAGE_END();
	}
