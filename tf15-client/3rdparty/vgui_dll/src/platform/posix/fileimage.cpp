	...
	FileImage *pImage)
{
	...
	// Read and verify the header. TGA fields are little-endian; this used
	// to trust the raw struct read as-is, corrupting m_Width/m_Height on
	// this big-endian PPU target and driving the allocation below off a
	// garbage size (same bug class as DataInputStream.cpp's readUShort).
	fp->Read(&hdr, sizeof(hdr));
	hdr.m_CMapStart = LittleWord(hdr.m_CMapStart);
	hdr.m_CMapLength = LittleWord(hdr.m_CMapLength);
	hdr.m_XOffset = LittleWord(hdr.m_XOffset);
	hdr.m_YOffset = LittleWord(hdr.m_YOffset);
	hdr.m_Width = LittleWord(hdr.m_Width);
	hdr.m_Height = LittleWord(hdr.m_Height);
	...
	FileImage *pImage)
{
	...
	// TGA fields are little-endian on disk regardless of host order --
	// never called in this tree today, but symmetric with the Load fix
	// above rather than leaving a second landmine in the same file.
	hdr.m_Width = LittleWord( (unsigned short)pImage->m_Width );
	hdr.m_Height = LittleWord( (unsigned short)pImage->m_Height );
