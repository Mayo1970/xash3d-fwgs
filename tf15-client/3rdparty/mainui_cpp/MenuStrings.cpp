...
// PS3: resource files are always UTF-16LE on disk (BOM 0xFFFE). The code
// below reads each 16-bit unit in host byte order, which is correct on a
// little-endian host but silently corrupted every character on this
// big-endian one -- COM_ParseFile then failed its very first "lang" header
// check, Localize_AddToDictionary bailed via `goto error` before inserting
// a single token, and every L() lookup fell back to the raw token (symptom:
// "GameUI_Multiplayer" etc shown literally instead of localized text).
// LittleShort() (xash3d_types.h) already resolves to a no-op on
// little-endian hosts and a real byteswap on big-endian ones, so reuse it
// instead of adding a new byteswap primitive.
static void ByteSwapUTF16File( uint16_t *units, int count )
{
	for( int i = 0; i < count; i++ )
		units[i] = (uint16_t)LittleShort( (short)units[i] );
}
...
static void Localize_AddToDictionary( const char *name, const char *lang )
{
	...
	if( pFileBuf[0] == '\xFF' && pFileBuf[1] == '\xFE' )
	{
		...
		// byteswap the content (past the 2-byte BOM) in place, matching the
		// exact pointer/count the Q_UTF16ToUTF8 call below uses.
		ByteSwapUTF16File( (uint16_t *)&pFileBuf[2], ( len / 2 ) - 1 );
		isUtf16 = true;
	}
