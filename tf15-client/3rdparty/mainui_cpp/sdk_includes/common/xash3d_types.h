...
// PS3: no prior big-endian-non-MSVC build ever compiled this arm. `_inline`
// is an MSVC-only keyword (no macro defines it anywhere in this tree) --
// GCC/Clang only understand `inline`/`__inline`. This file is C++-only
// (only ever included from .cpp translation units here), so plain `inline`
// is valid and behavior-preserving.
