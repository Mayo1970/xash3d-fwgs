...
#include "const.h"
#include "usercmd.h"
// com_model.h before pm_defs.h: pm_defs.h's pmove_t only forward-references
// "struct hull_s *" (for PM_HullPointContents), so without this order GCC
// treats that forward-declared tag and com_model.h's later full "typedef
// struct hull_s {...} hull_t" as two distinct types with the same spelling
// -- "passing argument 1 ... from incompatible pointer type" even though
// the diagnostic prints both sides identically. Defining the real struct
// first avoids the two-declaration-point ambiguity entirely.
#include "com_model.h"
#include "pm_defs.h"
#include "pm_shared.h"
