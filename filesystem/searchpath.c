...
*/
#if XASH_PS3
static void FS_Prof_Report( const char *name, const char *result, double start,
	const fs_prof_t *before, uint32_t walked, const searchpath_t *worst, double worst_msec, double threshold )
{
	double total = FS_Prof_Time() - start;
	string worstinfo;

	if( total < threshold )
		return;

	if( worst && worst->pfnPrintInfo )
		worst->pfnPrintInfo( (searchpath_t *)worst, worstinfo, sizeof( worstinfo ));
	else Q_strncpy( worstinfo, "(none)", sizeof( worstinfo ));

	Con_Printf( "[fs/prof] %s \"%s\" %.2fms over %u searchpath(s)\n",
		result, name, total, walked );
	Con_Printf( "[fs/prof]   listdir %u call(s) %u entr(ies) %.2fms | stat %u %.2fms | open %u %.2fms | populate %u rescan %u refresh %u\n",
		fs_prof.listdir_calls - before->listdir_calls,
		fs_prof.listdir_entries - before->listdir_entries,
		fs_prof.listdir_msec - before->listdir_msec,
		fs_prof.stat_calls - before->stat_calls,
		fs_prof.stat_msec - before->stat_msec,
		fs_prof.open_calls - before->open_calls,
		fs_prof.open_msec - before->open_msec,
		fs_prof.populate_calls - before->populate_calls,
		fs_prof.rescan_calls - before->rescan_calls,
		fs_prof.refresh_calls - before->refresh_calls );
	Con_Printf( "[fs/prof]   worst searchpath %.2fms: %s\n", worst_msec, worstinfo );
	// cumulative totals: if the dir cache is doing its job these plateau, if
	// the same directories are being re-listed they keep climbing
	Con_Printf( "[fs/prof]   session total: listdir %u/%u %.0fms | stat %u %.0fms | open %u %.0fms | d_type reg %u dir %u unknown %u other %u\n",
		fs_prof.listdir_calls, fs_prof.listdir_entries, fs_prof.listdir_msec,
		fs_prof.stat_calls, fs_prof.stat_msec,
		fs_prof.open_calls, fs_prof.open_msec,
		fs_prof.dtype_reg, fs_prof.dtype_dir, fs_prof.dtype_unknown, fs_prof.dtype_other );
}
#endif // XASH_PS3
...
searchpath_t *FS_FindFile( const char *name, int *index, char *fixedname, size_t len, uint32_t flags )
{
	searchpath_t *search;
#if XASH_PS3
	// goal 14: attribute the 100-350ms per-name lookup cost to a concrete LV2
	// primitive. Snapshot the free-running counters around the whole walk and
	// separately around each searchpath, so a slow lookup names both the call
	// mix and the searchpath that paid for it.
	const double prof_threshold = 20.0; // msec
	fs_prof_t prof_before = fs_prof;
	double prof_start = FS_Prof_Time();
	double prof_worst_msec = 0.0;
	const searchpath_t *prof_worst = NULL;
	uint32_t prof_walked = 0;
#endif
	...
	for( search = fs_searchpaths; search; search = search->next )
	{
...
#if XASH_PS3
		prof_walked++;
		prof_sp_start = FS_Prof_Time();
#endif
		pack_ind = search->pfnFindFile( search, name, fixedname, len );
#if XASH_PS3
		prof_sp_msec = FS_Prof_Time() - prof_sp_start;
		if( prof_sp_msec > prof_worst_msec )
		{
			prof_worst_msec = prof_sp_msec;
			prof_worst = search;
		}
#endif
		if( pack_ind >= 0 )
		{
			if( index )
				*index = pack_ind;
#if XASH_PS3
			FS_Prof_Report( name, "hit", prof_start, &prof_before, prof_walked, prof_worst, prof_worst_msec, prof_threshold );
#endif
			return search;
		}
	}
...
#if XASH_PS3
	FS_Prof_Report( name, "miss", prof_start, &prof_before, prof_walked, prof_worst, prof_worst_msec, prof_threshold );
#endif
...
search_t *FS_Search( const char *pattern, int caseinsensitive, int gamedironly )
{
...
#if XASH_PS3
	// same attribution the FS_FindFile walk gets: FS_Search lists a directory
	// per searchpath per pattern, so a slow boot names the pattern that paid
	const double prof_threshold = 20.0; // msec
	fs_prof_t prof_before = fs_prof;
	double prof_start = FS_Prof_Time();
	double prof_worst_msec = 0.0;
	...
	{
#if XASH_PS3
		double prof_sp_start, prof_sp_msec;
#endif
		...
		{
			prof_worst_msec = prof_sp_msec;
			prof_worst = searchpath;
		}
#endif
	}

#if XASH_PS3
	FS_Prof_Report( pattern, "search", prof_start, &prof_before, prof_walked, prof_worst, prof_worst_msec, prof_threshold );
#endif
