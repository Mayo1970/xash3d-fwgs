...
file_t *FS_Open( const char *filepath, const char *mode, qboolean gamedironly )
{
	...
	if( mode[0] == 'w' || mode[0] == 'a'|| mode[0] == 'e' || Q_strchr( mode, '+' ))
	{
		...
		// this may have added a name to (or created) the directories along
...
qboolean FS_Rename( const char *oldname, const char *newname )
{
...
#if XASH_PS3
	{
		char ps3_oldpath[MAX_SYSPATH], ps3_newpath[MAX_SYSPATH];
		ret = rename( PS3_ResolvePath( oldpath, ps3_oldpath, sizeof( ps3_oldpath )),
			PS3_ResolvePath( newpath, ps3_newpath, sizeof( ps3_newpath )));
	}
#else
	...
	FS_InvalidateDirCache( fs_writepath->dir, oldname2 );
	FS_InvalidateDirCache( fs_writepath->dir, newname2 );
	return true;
}
...
qboolean GAME_EXPORT FS_Delete( const char *path )
{
...
#if XASH_PS3
	{
		char ps3_real_path[MAX_SYSPATH];
		ret = remove( PS3_ResolvePath( real_path, ps3_real_path, sizeof( ps3_real_path )));
	}
#else
	...
	if( ret == 0 )
		FS_InvalidateDirCache( fs_writepath->dir, path2 );
