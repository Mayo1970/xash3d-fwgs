...
// PSL1GHT has no dynamic loader: there is no dlopen/dlsym/dlclose and no
// dlfcn.h at all, so every module-loading half of this file is compiled out on
// PS3. What is left -- CreateInterface, Sys_GetFactoryThis, InitializeInterface
// -- is the file-scope interface registry, which is the only part this build
// uses (the one caller of Sys_LoadModule, FileSystem_Init, has its own PS3
// branch in FileSystem.cpp). Compiled out rather than stubbed on purpose: a
// stub returning nullptr would turn a future call site into a silent runtime
// failure, while leaving the symbol undefined makes it a link error instead.
// Same treatment as cs16-client/common/interface.cpp.
#ifndef __PPU__
