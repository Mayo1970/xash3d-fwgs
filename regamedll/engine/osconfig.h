...
// enable SSE code only if it's enabled in compiler options
	...
	#include <sys/stat.h>
	#include <io.h>
#elif defined __PPU__ // PS3 (PSL1GHT/newlib)
	// PSL1GHT's newlib ships no dlfcn.h/elf.h/link.h (no dynamic loader at
	// all), no pthread.h, no sys/ioctl.h, no sys/mman.h and no sys/sysinfo.h
	// on the PPU. Everything below is what this toolchain actually provides
	// and this tree actually uses; the dlopen-dependent code paths are
	// compiled out separately (see public/interface.cpp and
...
#include <xmmintrin.h>
	...
	// PS3's newlib has neither sys/ioctl.h nor sys/mman.h, and nothing in this
	// tree calls these three (checked) -- they are engine-side helpers that
	// come along with the shared osconfig.h. Compiled out rather than
	// reimplemented over malloc/free: a future caller should fail to link here
	// instead of silently getting different allocation semantics.
#ifndef __PPU__
