...
public:
	...
	// PS3: this module is compiled -fno-rtti (see regamedll/dlls/wscript --
	// xshlib.py localizes every symbol but the exports table, and localizing a
	// COMDAT typeinfo makes ld discard the group while references to it
	// survive), so the one dynamic_cast in this tree, in cbase.cpp's
	// OnFreeEntPrivateData, cannot be used. Replaced with the SDK's own
	// downcast idiom, the same one MyMonsterPointer/MySquadMonsterPointer
	// above already use: a virtual returning nullptr by default, overridden in
	// CBasePlayerItem to return itself. Added at the end of the virtual list
	// so no existing vtable slot moves.
	virtual CBasePlayerItem *MyItemPointer() { return nullptr; }
