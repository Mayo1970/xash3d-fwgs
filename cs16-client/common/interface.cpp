...
// Everything from here to Sys_GetFactoryThis() loads code at runtime, which
// PSL1GHT cannot do -- there is no dlopen and the game modules are linked
// straight into the EBOOT (see xshlib.py's --static-linking). Nothing in the
// client calls any of it, so it is compiled out rather than stubbed: a stub
// returning NULL would turn a future call site into a silent runtime failure,
// while leaving the symbol undefined makes it a link error instead.
#if !XASH_PS3
