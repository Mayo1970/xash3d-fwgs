...
int CHudHealth :: MsgFunc_ClCorpse(const char *pszName, int iSize, void *pbuf)
{
	...
	// szModel holds "models/player/<name>/<name>.mdl", so it must fit two
	// copies of a name that arrives over the network unbounded. Clamp the
	// name to szName first and size szModel to the worst case that produces
	// -- otherwise the format can only ever be truncated, which is a real
	// (if benign) wire-driven overflow of the old 64-byte buffer, not a
	// warning to wave through.
	char *pModel, szModel[96], szName[32];
	...
	if( !gHUD.cl_minmodels->value )
	{
		...
		{
			strlcpy( szName, pModel, sizeof( szName ) );
			snprintf( szModel, sizeof(szModel), "models/player/%s/%s.mdl", szName, szName );
		}
		else
