...
void CMenuSlider::Draw( void )
{
	...
	unsigned int thumbColor = ( eFocusAnimation == QM_HIGHLIGHTIFFOCUS && this == m_pParent->ItemAtCursor()) ?
		uiColorHelp : uiColorWhite;

	if( imgSlider.IsValid() )
		UI_DrawPic( sliderX, m_scPos.y, m_scCenterBox.w, m_scSize.h, thumbColor, imgSlider );
	else // gfx/shell/slider missing -- keep the thumb position visible as a plain filled block
		UI_FillRect( Point( sliderX, m_scPos.y ), m_scCenterBox, thumbColor );
