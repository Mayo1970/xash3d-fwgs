...
void CMenuCheckBox::Draw( void )
{
	...
	{
		if( szGrayedPic.IsValid() )
			UI_DrawPic( m_scPos, m_scSize, uiColorWhite, szGrayedPic );
		else
		{
			UI_DrawRectangle( m_scPos, m_scSize, uiColorDkGrey );
			if( bChecked )
				UI_FillRect( m_scPos, m_scSize, uiColorDkGrey );
		}
		return; // grayed
	}

	bool hasFocus = !((( iFlags & QMF_MOUSEONLY ) && !( iFlags & QMF_HASMOUSEFOCUS ))
	   || ( this != m_pParent->ItemAtCursor() ));

	// gfx/shell/cb_* textures may be missing (e.g. stripped platform asset sets) --
	// fall back to a plain outline+fill box so checked/focus state stays legible
	if( !szEmptyPic.IsValid() )
	{
		unsigned int boxColor = hasFocus ? colorFocus : colorBase;

		UI_DrawRectangle( m_scPos, m_scSize, boxColor );
		if( bChecked )
			UI_FillRect( m_scPos, m_scSize, boxColor );
		return;
	}
