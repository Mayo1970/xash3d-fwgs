...
// AGENTS.md goal-10 freeze hunt: rounds 8-10 narrowed the first-map-load
// hard lock to pfnPostRunCmd (this file's HUD_PostRunCmd) on the first
// ca_active frame. PS3_Diag lives in the engine (sys_ps3.c) and resolves at
// final static link, same as any other cross-module reference here.
#if defined( __PPU__ )
extern "C" void PS3_Diag( const char *fmt, ... );
...
void HUD_PrepEntity( CBaseEntity *pEntity, CBasePlayer *pWeaponOwner )
{
	...
	PS3_DIAG( "wpn:   PrepEntity slot=%i before Precache", num_ents - 1 );
	pEntity->Precache();
	PS3_DIAG( "wpn:   PrepEntity slot=%i before Spawn", num_ents - 1 );
	pEntity->Spawn();
	PS3_DIAG( "wpn:   PrepEntity slot=%i after Spawn", num_ents - 1 );
	...
	if( pWeaponOwner )
	{
		...
		PS3_DIAG( "wpn:   PrepEntity GetItemInfo iId=%i (MAX_WEAPONS=%i)", info.iId, (int)MAX_WEAPONS );
...
void HUD_InitClientWeapons( void )
{
	...
	initialized = 1;
	PS3_DIAG( "wpn: HUD_InitClientWeapons first call" );
	...
	PS3_DIAG( "wpn: PrepEntity player" );
	HUD_PrepEntity( &player, NULL );

	// Allocate slot(s) for each weapon that we are going to be predicting
	PS3_DIAG( "wpn: PrepEntity glock" );      HUD_PrepEntity( &g_Glock, &player );
	PS3_DIAG( "wpn: PrepEntity crowbar" );    HUD_PrepEntity( &g_Crowbar, &player );
	PS3_DIAG( "wpn: PrepEntity python" );     HUD_PrepEntity( &g_Python, &player );
	PS3_DIAG( "wpn: PrepEntity mp5" );        HUD_PrepEntity( &g_Mp5, &player );
	PS3_DIAG( "wpn: PrepEntity crossbow" );   HUD_PrepEntity( &g_Crossbow, &player );
	PS3_DIAG( "wpn: PrepEntity shotgun" );    HUD_PrepEntity( &g_Shotgun, &player );
	PS3_DIAG( "wpn: PrepEntity rpg" );        HUD_PrepEntity( &g_Rpg, &player );
	PS3_DIAG( "wpn: PrepEntity gauss" );      HUD_PrepEntity( &g_Gauss, &player );
	PS3_DIAG( "wpn: PrepEntity egon" );       HUD_PrepEntity( &g_Egon, &player );
	PS3_DIAG( "wpn: PrepEntity hgun" );       HUD_PrepEntity( &g_HGun, &player );
	PS3_DIAG( "wpn: PrepEntity handgren" );   HUD_PrepEntity( &g_HandGren, &player );
	PS3_DIAG( "wpn: PrepEntity satchel" );    HUD_PrepEntity( &g_Satchel, &player );
	PS3_DIAG( "wpn: PrepEntity tripmine" );   HUD_PrepEntity( &g_Tripmine, &player );
	PS3_DIAG( "wpn: PrepEntity snark" );      HUD_PrepEntity( &g_Snark, &player );
#ifdef OPFOR
	PS3_DIAG( "wpn: PrepEntity displacer" );  HUD_PrepEntity( &g_Displacer, &player );
	PS3_DIAG( "wpn: PrepEntity eagle" );      HUD_PrepEntity( &g_Eagle, &player );
	PS3_DIAG( "wpn: PrepEntity knife" );      HUD_PrepEntity( &g_Knife, &player );
	PS3_DIAG( "wpn: PrepEntity m249" );       HUD_PrepEntity( &g_M249, &player );
	PS3_DIAG( "wpn: PrepEntity penguin" );    HUD_PrepEntity( &g_Penguin, &player );
	PS3_DIAG( "wpn: PrepEntity pipewrench" ); HUD_PrepEntity( &g_PipeWrench, &player );
	PS3_DIAG( "wpn: PrepEntity shock" );      HUD_PrepEntity( &g_Shock, &player );
	PS3_DIAG( "wpn: PrepEntity sniper" );     HUD_PrepEntity( &g_Sniper, &player );
	PS3_DIAG( "wpn: PrepEntity spore" );      HUD_PrepEntity( &g_Spore, &player );
	PS3_DIAG( "wpn: PrepEntity grapple" );    HUD_PrepEntity( &g_Grapple, &player );
#endif // OPFOR
	PS3_DIAG( "wpn: HUD_InitClientWeapons done, num_ents=%i", num_ents );
}
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	PS3_DIAG( "wpn: WeaponsPostThink enter, from m_iId=%i", from->client.m_iId );
	HUD_InitClientWeapons();
	PS3_DIAG( "wpn: after HUD_InitClientWeapons" );
	...
	PS3_DIAG( "wpn: after weapon switch, pWeapon=%p", (void *)pWeapon );
	if( !pWeapon )
		return;
	...
	PS3_DIAG( "wpn: after g_pWpns from-loop" );
	...
	PS3_DIAG( "wpn: before m_pActiveItem RPG check, m_pActiveItem=%p", (void *)player.m_pActiveItem );
	...
	{
		PS3_DIAG( "wpn: before ItemPostFrame, m_flNextAttack_ms=%i", (int)( player.m_flNextAttack * 1000 ));
		if( player.m_flNextAttack <= 0 )
		{
			pWeapon->ItemPostFrame();
		}
		PS3_DIAG( "wpn: after ItemPostFrame" );
	}
	...
	PS3_DIAG( "wpn: after weaponselect block" );
	...
	PS3_DIAG( "wpn: second m_pActiveItem RPG check, m_pActiveItem=%p", (void *)player.m_pActiveItem );
	if( player.m_pActiveItem->m_iId == WEAPON_RPG )
	{
	...
	PS3_DIAG( "wpn: before g_pWpns to-loop" );
	for( i = 0; i < MAX_WEAPONS; i++ )
	{
		pCurrent = g_pWpns[i];
...
{
	g_runfuncs = runfuncs;
	PS3_DIAG( "wpn: HUD_PostRunCmd enter, runfuncs=%i cl_lw=%s value=%i", runfuncs,
		cl_lw ? cl_lw->string : "(null)", cl_lw ? (int)cl_lw->value : -1 );

#if CLIENT_WEAPONS
	if( cl_lw && cl_lw->value )
	{
		PS3_DIAG( "wpn: before HUD_WeaponsPostThink" );
		HUD_WeaponsPostThink( from, to, cmd, time, random_seed );
		PS3_DIAG( "wpn: after HUD_WeaponsPostThink" );
	}
	else
#endif
	{
	...
	PS3_DIAG( "wpn: HUD_PostRunCmd exit" );
}
