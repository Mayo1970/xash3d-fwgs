...
int DLLEXPORT HUD_Key_Event( int down, int keynum, const char *pszCurrentBinding )
{
	...
	// The stock GoldSrc numbered menu (team/class select, etc.) expects a
	// keyboard's 1-9/0 keys bound to "menuselect" -- the default gamepad
	// bind table has no such binds (every button is a permanent FPS action),
	// so on a console the menu draws but nothing can select from it. Map a
	// fixed set of pad buttons to the slots these menus actually use, but
	// only while a menu is on screen so gameplay binds are untouched.
	if( down && gHUD.m_Menu.m_fMenuDisplayed )
	{
		int slot = 0;

		// hlsdk-portable/engine/keydefs.h (a vendored SDK snapshot, distinct
		// from the engine's own copy) only has the raw K_AUXn numbers, not
		// the K_A_BUTTON/K_DPAD_* aliases -- use the numbers directly.
		switch( keynum )
		{
		case K_AUX16: slot = 1;  break; // DPAD_UP:    team 1 / class 1
		case K_AUX17: slot = 2;  break; // DPAD_DOWN:  team 2 / class 2
		case K_AUX1:  slot = 3;  break; // A_BUTTON:   VIP / class 3
		case K_AUX18: slot = 5;  break; // DPAD_LEFT:  auto-assign
		case K_AUX19: slot = 6;  break; // DPAD_RIGHT: spectate
		case K_AUX2:              // B_BUTTON
		case K_AUX9:  slot = 10; break; // START_BUTTON: cancel
		}

		if( slot && ( gHUD.m_Menu.m_bitsValidSlots & ( 1 << ( slot - 1 ) ) ) )
		{
			gHUD.m_Menu.SelectMenuItem( slot );
			return 0;
		}
	}
