...
#if XASH_PS3
/*
 * Goal 16: the engine measures this class as one opaque `client` bucket
 * (2.5-3.1ms/frame, the largest single studio cost after the host-ring fix)
 * because it derives it by subtraction at its own dispatch point -- the code
 * itself lives here, in the game DLL, where nothing could see inside it.
 *
 * ps3gl_ticks() is a single mftb, not gettimeofday(); extern "C" because ps3gl
 * is a C module and this TU is C++. Flushed once per model at the end of each
 * Studio*Draw* entry point.
 */
extern "C" unsigned long long ps3gl_ticks( void );
extern "C" void ps3gl_report_studio_client( unsigned long long xform,
	unsigned long long bones, unsigned long long events );

#define PS3_CT( var )		unsigned long long var = ps3gl_ticks()
#define PS3_CEND( var, acc )	(( acc ) += ps3gl_ticks() - ( var ))
#else
#define PS3_CT( var )		((void)0)
#define PS3_CEND( var, acc )	((void)0)
#endif
...
int CStudioModelRenderer::StudioDrawModel( int flags )
{
	...
	unsigned long long ps3_xform = 0, ps3_bones = 0, ps3_events = 0;
...
#if XASH_PS3
	ps3gl_report_studio_client( ps3_xform, ps3_bones, ps3_events );
#endif
	return 1;
}
...
int CStudioModelRenderer::StudioDrawPlayer( int flags, entity_state_t *pplayer )
{
	...
	unsigned long long ps3_xform = 0, ps3_bones = 0, ps3_events = 0;
	PS3_CT( ps3_t_xform );	// gait + transform setup, closed after the if/else
	...
	PS3_CEND( ps3_t_xform, ps3_xform );
