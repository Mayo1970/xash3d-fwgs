...
// AGENTS.md goal-10 freeze hunt: round 11 put the first-map-load hard lock
// inside this function (client-side HUD_PrepEntity -> g_Glock.Spawn()). The
// first marker also proves whether the virtual dispatch even arrives here,
// which separates "a statement below faults" from "the vtable/static-ctor
// path did not survive the ld -r + objcopy -G static-link merge".
#if defined( __PPU__ )
...
void CGlock::Spawn()
{
	PS3_DIAG( "glock: Spawn entered, pev=%p", (void *)pev );
	pev->classname = MAKE_STRING( "weapon_9mmhandgun" ); // hack to allow for old names
	PS3_DIAG( "glock: after MAKE_STRING classname" );
	Precache();
	PS3_DIAG( "glock: after Precache" );
	m_iId = WEAPON_GLOCK;
	PS3_DIAG( "glock: before SET_MODEL, ENT(pev)=%p", (void *)ENT( pev ));
	SET_MODEL( ENT( pev ), "models/w_9mmhandgun.mdl" );
	PS3_DIAG( "glock: after SET_MODEL" );

	m_iDefaultAmmo = GLOCK_DEFAULT_GIVE;

	FallInit();// get ready to fall down.
	PS3_DIAG( "glock: after FallInit, Spawn exit" );
}
