...
// Opposing Force drew the CTF team picker through VGUI, so the "#Team_Menu_Join"
// token this non-VGUI path used was never defined in any shipping titles.txt --
// LocaliseTextString() misses and prints the raw token. Send the literal menu
// body instead so it does not depend on game data that does not exist.
#define CTF_TEAM_MENU_TEXT \
	"\\yCapture The Flag\n\n" \
	"\\w1. Black Mesa\n" \
	"\\w2. Opposing Force\n\n" \
	"\\w5. Auto Assign\n"

#define CTF_TEAM_MENU_TEXT_IG \
	"\\yChange Team\n\n" \
	"\\w1. Black Mesa\n" \
	"\\w2. Opposing Force\n\n" \
	"\\w5. Auto Assign\n\n" \
...
BOOL CCTFMultiplay::ClientCommand(CBasePlayer *pPlayer, const char *pcmd)
{
	...
	else if (FStrEq(pcmd, "changeteam"))
	{
		if (pPlayer->pev->team != 0)
		{
			pPlayer->ShowMenu(1 + 2 + 16 + 512, -1, FALSE, CTF_TEAM_MENU_TEXT_IG);
...
void CCTFMultiplay::InitHUD(CBasePlayer *pPlayer)
{
	...
	if (pPlayer->pev->team == 0)
	{
		pPlayer->ShowMenu(1 + 2 + 16, -1, FALSE, CTF_TEAM_MENU_TEXT);
