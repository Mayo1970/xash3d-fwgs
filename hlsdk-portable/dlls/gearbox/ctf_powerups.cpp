...
static const char* GetRunePrintName( int flag )
{
	if (flag & IT_CTF_ACCELERATOR_FLAG)
		return "Accelerator";
	if (flag & IT_CTF_BACKPACK_FLAG)
		return "Backpack";
	if (flag & IT_CTF_LONGJUMP_FLAG)
		return "Long jump";
	if (flag & IT_CTF_PORTABLEHEV_FLAG)
		return "Portable HEV";
	if (flag & IT_CTF_REGEN_FLAG)
		return "Regeneration";
...
void CPowerupCTFBase::Spawn(void)
{
	...
	if (GetRuneFlag() < 0)
	{
		...
		{
			m_iszPrintName = MAKE_STRING(GetRunePrintName(flag));
			m_iRuneFlag = flag;
		}
	...
	else
	{
		...
		m_iszPrintName = MAKE_STRING(GetRunePrintName(m_iRuneFlag));
	}

	if (m_iRuneFlag < 0 || FStringNull(pev->model))
	{
...
#if 1
	ALERT(at_console, "Spawning rune %d (%s) with model %s.\n", m_iRuneFlag, STRING(m_iszPrintName), STRING(pev->model));
#endif
