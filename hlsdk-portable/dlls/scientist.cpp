...
#ifdef BSHIFT
// Dr. Rosenberg reuses CScientist wholesale; every behavioural difference
// below is selected at runtime off the classname.
...
int CScientist::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	{
#ifdef BSHIFT
		// Rosenberg is plot-critical -- he never turns on the player.
		if( !FClassnameIs( pev, "monster_rosenberg" ) )
#endif
		{
...
void CScientist::PainSound( void )
{
	...
	if( FClassnameIs( pev, "monster_rosenberg" ) )
	{
		// Rosenberg ships nine pain lines against the scientist's five, so
		// pick the path by name rather than widening the shared table.
		static const char *const pszRosenbergPain[] =
		{
		...
		EMIT_SOUND_DYN( ENT( pev ), CHAN_VOICE, pszRosenbergPain[RANDOM_LONG( 0, 8 )], 1, ATTN_NORM, 0, GetVoicePitch() );
		return;
	}
#endif
...
#ifdef BSHIFT
			// Rosenberg never flees the player.
			else if( !FClassnameIs( pev, "monster_rosenberg" ) )
#else
