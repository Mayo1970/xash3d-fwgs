...
CVAR_DEFINE_AUTO( sv_baseline_search_depth, "8", 0, "how many previous entity states SV_FindBestBaseline may scan (0 = off, 63 = stock)" );
...
void Host_ServerFrame( void )
{
	...
	PS3_DIAG_FRAME( "sv: before SV_ReadPackets" );
	// read packets from clients
	{
		double ps3_t = PS3_PHASE_NOW();
		SV_ReadPackets ();
		PS3_PHASE_ADD( ps3_phase_sv_readpackets, ps3_t );
	}
	PS3_DIAG_FRAME( "sv: after SV_ReadPackets" );
	...
	PS3_DIAG_FRAME( "sv: before SV_RunGameFrame, sv.simulating=%i", sv.simulating );
	// let everything in the world think and move
	{
		double ps3_t = PS3_PHASE_NOW();
		qboolean ps3_ran = SV_RunGameFrame ();
		PS3_PHASE_ADD( ps3_phase_sv_gameframe, ps3_t );

		if( !ps3_ran )
		{
			PS3_DIAG_FRAME( "sv: SV_RunGameFrame returned false, early return" );
			return;
		}
	}
	PS3_DIAG_FRAME( "sv: after SV_RunGameFrame" );

	// send messages back to the clients that had packets read this frame
	{
		double ps3_t = PS3_PHASE_NOW();
		SV_SendClientMessages ();
		PS3_PHASE_ADD( ps3_phase_sv_sendmsgs, ps3_t );
	}
	PS3_DIAG_FRAME( "sv: after SV_SendClientMessages" );

	// clear edict flags for next frame
	SV_PrepWorldFrame ();
	PS3_DIAG_FRAME( "sv: after SV_PrepWorldFrame" );
...
void SV_Init( void )
{
	...
	Cvar_RegisterVariable( &sv_baseline_search_depth );
