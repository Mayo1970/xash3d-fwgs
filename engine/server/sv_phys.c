...
static qboolean SV_RunThink( edict_t *ent )
{
	...
	if( !FBitSet( ent->v.flags, FL_KILLME ))
	{
		...
		// timed: this is the whole game DLL think for one entity, so an
		// env_rope's RopeThink (sim + per-segment traces) lands here
		PS3_PHASE_SCOPE( ps3_phase_sv_think, ps3_phase_sv_think_calls,
