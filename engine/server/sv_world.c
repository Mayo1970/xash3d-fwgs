...
*/
#if XASH_PS3
// Timed wrapper: SV_LinkEdict is called from a dozen physics sites and is
// also exported to the game DLL, so wrapping here catches every caller
// rather than patching each one. The real body is SV_LinkEdict_Timed below.
static void SV_LinkEdict_Timed( edict_t *ent, qboolean touch_triggers );

void GAME_EXPORT SV_LinkEdict( edict_t *ent, qboolean touch_triggers )
{
	PS3_PHASE_SCOPE( ps3_phase_sv_link, ps3_phase_sv_link_calls,
		SV_LinkEdict_Timed( ent, touch_triggers ));
}
