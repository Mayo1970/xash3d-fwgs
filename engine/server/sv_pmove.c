...
void SV_RunCmd( sv_client_t *cl, usercmd_t *ucmd, int random_seed )
{
	...
	double	frametime;
	usercmd_t cmd;
	PS3_DIAG_FRAME( "sv: SV_RunCmd enter, msec=%i", ucmd->msec );
	...
	PS3_DIAG_FRAME( "sv: before pfnCmdStart" );
	svgame.dllFuncs.pfnCmdStart( cl->edict, ucmd, random_seed );
	PS3_DIAG_FRAME( "sv: after pfnCmdStart" );
	...
	PS3_DIAG_FRAME( "sv: before pfnPlayerPreThink" );
	svgame.dllFuncs.pfnPlayerPreThink( clent );
	PS3_DIAG_FRAME( "sv: after pfnPlayerPreThink, before SV_PlayerRunThink" );
	SV_PlayerRunThink( clent, frametime, cl->timebase );
	PS3_DIAG_FRAME( "sv: after SV_PlayerRunThink" );
	...
	PS3_DIAG_FRAME( "sv: before pfnPM_Move" );
	// motor!
	svgame.dllFuncs.pfnPM_Move( svgame.pmove, true );
	PS3_DIAG_FRAME( "sv: after pfnPM_Move, before SV_FinishPMove" );

	// copy results back to client
	SV_FinishPMove( svgame.pmove, cl );
	PS3_DIAG_FRAME( "sv: after SV_FinishPMove" );
