...
static void DirectoryCopy( const char *pPath, file_t *pFile )
{
	...
	for( int i = 0; i < t->numfilenames; i++ )
	{
		...
		{
			int fileSize_wire = LittleLong( fileSize );
			FS_Write( pFile, &fileSize_wire, sizeof( int ));
		}
...
static int GetClientDataSize( const char *level )
{
	...
	tokenCount = LittleLong( tokenCount );
	FS_Read( pFile, &tokenSize, sizeof( int ));
	tokenSize = LittleLong( tokenSize );
	FS_Close( pFile );
...
static SAVERESTOREDATA *LoadSaveData( const char *level )
{
	...
	tableCount = LittleLong( tableCount );
...
static qboolean EntityPatchWrite( SAVERESTOREDATA *pSaveData, const char *level )
{
	...
	// patch count
	{
		...
		FS_Write( pFile, &size_wire, sizeof( int ));
	}
	...
	for( i = 0; i < pSaveData->tableCount; i++ )
	{
		if( FBitSet( pSaveData->pTable[i].flags, FENTTABLE_REMOVED ))
		{
			int i_wire = LittleLong( i );
			FS_Write( pFile, &i_wire, sizeof( int ));
		}
	}
...
static qboolean SaveClientState( SAVERESTOREDATA *pSaveData, const char *level, int changelevel )
{
	...
	wire = LittleLong( id );
	FS_Write( pFile, &wire, sizeof( wire ));
	wire = LittleLong( version );
	FS_Write( pFile, &wire, sizeof( wire ));
	wire = LittleLong( pSaveData->size );
	FS_Write( pFile, &wire, sizeof( int )); // does not include token table

	// write out the tokens first so we can load them before we load the entities
	wire = LittleLong( pSaveData->tokenCount );
	FS_Write( pFile, &wire, sizeof( int ));
	wire = LittleLong( pSaveData->tokenSize );
	FS_Write( pFile, &wire, sizeof( int ));
...
static SAVERESTOREDATA *SaveGameState( int changelevel )
{
	...
	// write the header
	{
		...
		wire = LittleLong( pSaveData->size );
		FS_Write( pFile, &wire, sizeof( int ));	// total size of all data to initialize read buffer
		wire = LittleLong( pSaveData->tableCount );
		FS_Write( pFile, &wire, sizeof( int ));	// entities count to right initialize entity table
		wire = LittleLong( pSaveData->tokenCount );
		FS_Write( pFile, &wire, sizeof( int ));	// num hash tokens to prepare token table
		wire = LittleLong( pSaveData->tokenSize );
		FS_Write( pFile, &wire, sizeof( int ));	// total size of hash tokens
	}
...
static void LoadAdjacentEnts( const char *pOldLevel, const char *pLandmarkName )
{
	...
	// build the adjacent map list
	svgame.dllFuncs.pfnParmsChangeLevel();
	PS3_DIAG( "sv: LA0 pfnParmsChangeLevel done, connectionCount=%i", currentLevelData.connectionCount );

	for( i = 0; i < currentLevelData.connectionCount; i++ )
	{
		PS3_DIAG( "sv: LA1 i=%i map=%s", i, currentLevelData.levelList[i].mapName );
		...
		PS3_DIAG( "sv: LA2 LoadSaveData -> %p", pSaveData );
		...
		if( pSaveData )
		{
			...
			PS3_DIAG( "sv: LA3 before LoadClientState, movedCount=%i", movedCount );
			LoadClientState( pSaveData, currentLevelData.levelList[i].mapName, true, true );
			PS3_DIAG( "sv: LA4 after LoadClientState" );
	...
	PS3_DIAG( "sv: LA5 LoadAdjacentEnts done, foundprevious=%i", (int)foundprevious );
...
void SV_ChangeLevel( qboolean loadfromsavedgame, const char *mapname, const char *start, qboolean background )
{
	...
	PS3_DIAG( "sv: CL0 SV_ChangeLevel enter old=%s new=%s start=%s loadsaved=%i",
		oldlevel, level, startspot ? startspot : "(null)", (int)loadfromsavedgame );
	...
	if( loadfromsavedgame )
	{
		...
		PS3_DIAG( "sv: CL1 SaveGameState -> %p", pSaveData );
	...
	PS3_DIAG( "sv: CL2 before SV_InactivateClients" );
	SV_InactivateClients ();
	PS3_DIAG( "sv: CL3 before SV_FinalMessage" );
	SV_FinalMessage( "", true );
	PS3_DIAG( "sv: CL4 before SV_DeactivateServer" );
	SV_DeactivateServer ();

	PS3_DIAG( "sv: CL5 before SV_SpawnServer" );
	if( !SV_SpawnServer( level, startspot, background ))
		return;	// ???
	PS3_DIAG( "sv: CL6 after SV_SpawnServer" );

	if( loadfromsavedgame )
	{
		// finish saving gamestate
		SaveFinish( pSaveData );
		PS3_DIAG( "sv: CL7 after SaveFinish, before LoadGameState" );

		if( !LoadGameState( level, true ))
		{
			PS3_DIAG( "sv: CL8 LoadGameState failed, before SV_SpawnEntities" );
			SV_SpawnEntities( level );
		}
		PS3_DIAG( "sv: CL9 after LoadGameState/SV_SpawnEntities, before LoadAdjacentEnts" );
		LoadAdjacentEnts( oldlevel, startspot );
		PS3_DIAG( "sv: CL10 after LoadAdjacentEnts" );

		if( sv_newunit.value )
			ClearSaveDir();
		PS3_DIAG( "sv: CL11 before SV_ActivateServer (loadgame branch)" );
	...
	else
	{
		...
		svgame.dllFuncs.pfnResetGlobalState();
		PS3_DIAG( "sv: CL9b after pfnResetGlobalState, before SV_SpawnEntities" );
		SV_SpawnEntities( level );
		PS3_DIAG( "sv: CL11b after SV_SpawnEntities, before SV_ActivateServer (classic branch)" );
		SV_ActivateServer( true );
	}

	PS3_DIAG( "sv: CL12 SV_ChangeLevel complete" );
}
