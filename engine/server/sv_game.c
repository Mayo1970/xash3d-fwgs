...
static void SV_LoadFromFile( const char *mapname, char *entities )
{
	...
	if( !svgame.physFuncs.SV_LoadEntities || !svgame.physFuncs.SV_LoadEntities( mapname, entities ))
	{
		...
		while(( entities = COM_ParseFile( entities, token, sizeof( token ))) != NULL )
		{
			...
			// map-gated arming (sv_ps3diag_map) makes this affordable again:
			// names the exact entity whose pfnSpawn never returns, if that's
			// where a transition softlock is
			PS3_DIAG( "sv: E%d %s spawn", NUM_FOR_EDICT( ent ), SV_ClassName( ent ));
			...
			PS3_DIAG( "sv: E%d %s spawned", NUM_FOR_EDICT( ent ), SV_ClassName( ent ));
		}

		PS3_DIAG( "sv: SV_LoadFromFile entity loop complete, inhibited=%i", inhibited );
...
#if XASH_PS3
	PS3_Printf( "SV_UnloadProgs: enter, hInstance=%p\n", svgame.hInstance );
#endif
...
qboolean SV_LoadProgs( const char *name )
{
	...
	{
#if XASH_PS3
		PS3_Printf( "SV_LoadProgs: early return, already loaded, hInstance=%p\n", svgame.hInstance );
#endif
		return true;
	}
...
#if XASH_PS3
	{
		// these three markers are unconditional (bypass the PS3_DIAG budget)
		// because a second pfnGameInit call would mean svgame got zeroed
		// mid-game -- a much bigger bug than the transition softlock this
		// channel was armed to find, and it needs to be visible regardless
		// of whether the diag channel happens to be armed for this map
		static int gameinit_call_count = 0;
		PS3_Printf( "SV_LoadProgs: pfnGameInit call #%d, hInstance=%p\n", ++gameinit_call_count, svgame.hInstance );
	}
#endif
