...
#if XASH_PS3
// map-gated arming for the PS3_DIAG channel: rather than a global
// PS3_DIAG_ENABLE() (which spends the whole budget on whatever map happens
// to load first), point this at the map whose spawn/transition you're
// hunting a freeze in. sv_ps3diag_frames additionally arms the per-frame
// sites (PS3_DIAG_FRAME) -- leave it 0 unless the freeze needs frame-level
// resolution, since a stalled-but-not-hung frame loop will otherwise burn
// the budget before the map-load sequence even finishes.
...
static void SV_PS3DiagArm( const char *mapname )
{
	...
	PS3_DIAG_REARM();
	ps3_diag_frames = (int)sv_ps3diag_frames.value;
	PS3_Printf( "PS3_DIAG: armed for map \"%s\", frames=%i, budget=%i\n", mapname, ps3_diag_frames, ps3_diag_budget );
}
#endif // XASH_PS3
...
#if XASH_PS3
				// VOX sentence words (see tools/gen_reslist) are looked up
				// client-side WITHOUT an extension: VOX_LoadSound
				// (engine/client/sound/s_vox.c) builds its S_FindName key as
				// szpath + bare word, never appending ".wav". An extensioned
				// reslist.txt/.res entry for one of these silently precaches
				// a SEPARATE, never-looked-up sfx_t while VOX_LoadWord still
				// misses and falls through to a synchronous disk load --
				// confirmed on real hardware: precaching "holo/tr_holo_
				// intro.wav" via reslist.txt did not stop the live 43-320ms
				// late-precache stall for the VOX-played "holo/tr_holo_intro"
				// (no extension).
				//
				// But unconditionally stripping broke ordinary extensioned
				// .res entries (e.g. c0a0.res's "barney/teamup2.wav"): the
				// stripped bare name doesn't exist on disk, so the client's
				// FS_FileExists check in CL_EstimateNeededResources flags it
				// RES_WASMISSING and CL_ParseResourceList zeroes the sound
				// slot outright -- confirmed on real hardware, ~250-sound
				// "Could not load sound" burst restoring c0a0.res. Only
				// strip when the extensioned path is NOT a real file (the
				// VOX bare-word case); leave genuine extensioned entries
				// alone so SV_SoundIndex stores the exact string the normal
				// EMIT_SOUND-style precache path also uses.
				string stripped;
				Q_strncpy( stripped, filepath, sizeof( stripped ));
				COM_StripExtension( stripped );

				if( !FS_FileExists( token, false ) && FS_FileExists( va( DEFAULT_SOUNDPATH "%s", stripped ), false ))
					SV_SoundIndex( stripped );
				else
...
void SV_ActivateServer( int runPhysics )
{
	...
	PS3_DIAG( "sv: AS0 SV_ActivateServer enter, runPhysics=%i", runPhysics );
	...
	PS3_DIAG( "sv: AS1 before SV_FreeOldEntities" );
	SV_FreeOldEntities ();

	// Activate the DLL server code
	svgame.globals->time = sv.time;
	PS3_DIAG( "sv: AS2 before pfnServerActivate, numEntities=%i maxclients=%i", svgame.numEntities, svs.maxclients );
	svgame.dllFuncs.pfnServerActivate( svgame.edicts, svgame.numEntities, svs.maxclients );
	PS3_DIAG( "sv: AS3 after pfnServerActivate" );

	SV_SetStringArrayMode( true );

	// parse user-specified resources
	PS3_DIAG( "sv: AS4 before SV_CreateGenericResources" );
	SV_CreateGenericResources();
	PS3_DIAG( "sv: AS5 after SV_CreateGenericResources" );
	...
	PS3_DIAG( "sv: AS6 before settle loop, numFrames=%i", numFrames );
	for( i = 0; i < numFrames; i++ )
		SV_Physics();
	PS3_DIAG( "sv: AS7 after settle loop" );

	// create a baseline for more efficient communications
	SV_CreateBaseline();
	PS3_DIAG( "sv: AS8 after SV_CreateBaseline" );

	// collect all info from precached resources
	SV_CreateResourceList();
	PS3_DIAG( "sv: AS9 after SV_CreateResourceList" );

	// check and count all files that marked by user as unmodified (typically is a player models etc)
	SV_TransferConsistencyInfo();
	PS3_DIAG( "sv: AS10 after SV_TransferConsistencyInfo" );
	...
	PS3_DIAG( "sv: AS11 ss_active, SV_ActivateServer complete" );
...
static void SV_SetupClients( void )
{
	...
	svs.maxclients = requested_maxclients = (int)sv_maxclients.value;
	...
	{
		Con_Printf( S_NOTE "%s: capped maxplayers %d -> %d (platform limit)\n",
			__func__, requested_maxclients, svs.maxclients );
	}
...
qboolean SV_SpawnServer( const char *mapname, const char *startspot, qboolean background )
{
	...
	// AGENTS.md goal-10 freeze hunt: the PS3_DIAG markers across the engine,
	// ref_soft and hlsdk are left in place but the channel ships disabled --
	// set sv_ps3diag_map to arm it for a debugging session on a specific map.
#if XASH_PS3
	SV_PS3DiagArm( mapname );
	// item 7/1 of the 2026-07-27 break-point review: every map spawn is a
	// real data point for "largest contiguous block right now" -- in
	// particular, connecting to a real MP server right after a singleplayer
	// session reaches this same call for the new map, giving a genuine
	// post-SP-session number instead of guessing at fragmentation.
	PS3_ProbeMemory( mapname );
#endif
	PS3_DIAG( "SV_SpawnServer: %s", mapname );
