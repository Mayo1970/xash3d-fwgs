...
static void SV_AddEntitiesToPacket( edict_t *pViewEnt, edict_t *pClient, client_frame_t *frame, sv_ents_t *ents, qboolean from_client )
{
	...
	PS3_DIAG_FRAME( "sv: before pfnSetupVisibility" );
	svgame.dllFuncs.pfnSetupVisibility( pViewEnt, pClient, &clientpvs, &clientphs );
	if( !clientpvs ) fullvis = true;
	PS3_DIAG_FRAME( "sv: after pfnSetupVisibility, numEntities=%i", svgame.numEntities );
	...
	for( int e = 1; e < svgame.numEntities; e++ )
	{
		...
		// (per-entity pfnAddToFullPack markers removed: round 5 proved this
		// loop clean for all 116 c0a0 entities, and one marker per entity
		// per frame would exhaust the global diagnostic budget before the
		// render path -- the actual unexplored region -- is reached)

		// add entity to the net packet
		// counted (not timed): one call per entity per client per frame, so
		// the count tells us how many entities this walk actually visits
		ps3_phase_sv_fullpack_calls++;
...
int SV_FindBestBaseline( int index, entity_state_t **baseline, entity_state_t *to, client_frame_t *frame, qboolean player )
{
	...
	// Telemetry: this search is O(63 backward states x 52 fields) per newly
	// visible entity, since Delta_TestBaseline re-runs the whole field
	// compare loop. Measured emit was 14.92ms while the final write pass
	// only visits 6417 fields/frame -- the gap should be this search. These
	// count it directly instead of inferring it.
	ps3_baseline_calls++;
	...
	// How far back this search may scan. Stock scans MAX_CUSTOM_BASELINES-1
	// (63) previous states, and every step re-runs Delta_TestBaseline's full
	// ~52-field compare loop. Measured on PS3 hardware in the OpFor rope
	// areas: ~100 new entities/frame x ~55 tests each = ~285000 field
	// compares/frame = ~15ms, 59% of the frame. The instanced-baseline fast
	// path that would skip all of it can never trigger, because HLSDK ships
	// CreateInstancedBaselines() commented out, so sv.num_instanced is
	// always 0.
	//
	// The search only ever saves a few bytes per new entity; on a listen
	// server talking to a local client that is a catastrophic trade. Bound
	// it. Depth 0 disables the search entirely (always delta from the
	// entity's own baseline); the clamp ceiling reproduces stock exactly,
	// so this can only ever scan less than stock, never more.
	//
	// This is purely a compression-efficiency knob, not a protocol change:
	// the chosen offset is transmitted in the message and the client decodes
	// against whatever baseline the offset names, so any depth is correct on
	// the wire -- a smaller one just picks a slightly less optimal baseline.
	int search_depth = (int)sv_baseline_search_depth.value;

	if( search_depth < 0 )
		search_depth = 0;
	if( search_depth > MAX_CUSTOM_BASELINES - 2 )
		search_depth = MAX_CUSTOM_BASELINES - 2;

	// lookup backward for previous states and try to interpret current delta as baseline
	for( int i = index - 1; bestBitCount > 0 && i >= 0 && ( index - i ) <= search_depth; i-- )
	{
...
static void SV_EmitPacketEntities( sv_client_t *cl, client_frame_t *to, sizebuf_t *msg )
{
	...
	PS3_DIAG_FRAME( "sv: SV_EmitPacketEntities loop start, to->num_entities=%i oldmax=%i delta_sequence=%i",
		to->num_entities, oldmax, cl->delta_sequence );
	while( newindex < to->num_entities || oldindex < oldmax )
	{
		...
		if( newnum < oldnum )
		{
			...
			// (per-entity markers removed here: round 6 proved the whole
			// baseline-select / MSG_WriteDeltaEntity pass clean, and they
			// would exhaust the global budget -- see the note in
			// SV_AddEntitiesToPacket above)
			// count entities arriving as "new" this frame -- these are the
			// only ones that can pay for a baseline search at all
			ps3_baseline_newents++;
			ps3_baseline_num_instanced = sv.num_instanced;
...
static void SV_WriteClientdataToMessage( sv_client_t *cl, sizebuf_t *msg )
{
	...
	PS3_DIAG_FRAME( "sv: before pfnGetWeaponData check, FCL_LOCAL_WEAPONS=%i",
		FBitSet( cl->flags, FCL_LOCAL_WEAPONS ) ? 1 : 0 );
	if( FBitSet( cl->flags, FCL_LOCAL_WEAPONS ) && svgame.dllFuncs.pfnGetWeaponData( clent, frame->weapondata ))
	{
		PS3_DIAG_FRAME( "sv: after pfnGetWeaponData (true branch)" );
...
static void SV_WriteEntitiesToClient( sv_client_t *cl, sizebuf_t *msg )
{
	...
	// timed at the top-level call only -- this function recurses for portals,
	// so timing it inside would double-count the nested passes
	{
		double ps3_t = PS3_PHASE_NOW();
		SV_AddEntitiesToPacket( cl->pViewEntity, cl->edict, frame, &frame_ents, true );
		PS3_PHASE_ADD( ps3_phase_sv_addents, ps3_t );
		ps3_phase_sv_addents_calls++;
	}
	...
	PS3_DIAG_FRAME( "sv: before SV_EmitPacketEntities" );
	{
		double ps3_t = PS3_PHASE_NOW();
		SV_EmitPacketEntities( cl, frame, msg );
		PS3_PHASE_ADD( ps3_phase_sv_emit, ps3_t );
	}
	PS3_DIAG_FRAME( "sv: after SV_EmitPacketEntities" );
	SV_EmitEvents( cl, frame, msg );
	PS3_DIAG_FRAME( "sv: after SV_EmitEvents, send_pings=%i", send_pings );
	if( send_pings ) SV_EmitPings( msg );
	PS3_DIAG_FRAME( "sv: SV_WriteEntitiesToClient exit" );
}
...
static void SV_SendClientDatagram( sv_client_t *cl )
{
	...
	PS3_DIAG_FRAME( "sv: SV_SendClientDatagram after SV_WriteEntitiesToClient" );
	...
	PS3_DIAG_FRAME( "sv: SV_SendClientDatagram before Netchan_TransmitBits" );
	// send the datagram
	Netchan_TransmitBits( &cl->netchan, MSG_GetNumBitsWritten( &msg ), MSG_GetData( &msg ));
	PS3_DIAG_FRAME( "sv: SV_SendClientDatagram after Netchan_TransmitBits, exit" );
}
