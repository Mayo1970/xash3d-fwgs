...
// the native backend (avi_native.c) is a full replacement: it brings its own
// movie_state_s and its own copy of the shared state/init tail below, which
// can't be hoisted into a common file because that tail declares
// `static movie_state_t avi[2]` and so needs the complete struct
