...
static int FS_SeekOggVorbis( void *datasource, ogg_int64_t offset, int whence )
{
	vorbis_streaming_ctx_t *ctx = (vorbis_streaming_ctx_t *)datasource;
...
// on some ps3dev portlib builds ogg_int64_t is "long long" while
// OggFilestream_Seek (shared with the Opus path, which expects int64_t)
// uses plain int64_t -- on LP64 targets those are different types of the
// same width, so ov_callbacks.seek_func needs an exact-signature adapter.
static int OggFilestream_SeekVorbis( void *datasource, ogg_int64_t offset, int whence )
{
	return OggFilestream_Seek( datasource, (int64_t)offset, whence );
}

static const ov_callbacks ov_callbacks_membuf = {
	OggFilestream_Read,
	OggFilestream_SeekVorbis,
	NULL,
	OggFilestream_Tell
};
