...
static void CL_RunUsercmd( local_state_t *from, local_state_t *to, usercmd_t *u, qboolean runfuncs, double *time, unsigned int random_seed )
{
	...
	*to = *from;
	...
	if( CL_IsPredicted( ))
	{
		...
		PS3_DIAG_FRAME( "pred: RunUsercmd before CL_SetupPMove" );
		CL_SetupPMove( clgame.pmove, from, &cmd, runfuncs, *time );

		// motor!
		PS3_DIAG_FRAME( "pred: RunUsercmd before pfnPlayerMove" );
		clgame.dllFuncs.pfnPlayerMove( clgame.pmove, false );
		PS3_DIAG_FRAME( "pred: RunUsercmd after pfnPlayerMove" );

		// copy results back to client
		CL_FinishPMove( clgame.pmove, to );
		PS3_DIAG_FRAME( "pred: RunUsercmd after CL_FinishPMove" );
	...
	PS3_DIAG_FRAME( "pred: RunUsercmd before pfnPostRunCmd, runfuncs=%i", (int)runfuncs );
	clgame.dllFuncs.pfnPostRunCmd( from, to, &cmd, runfuncs, *time, random_seed );
	PS3_DIAG_FRAME( "pred: RunUsercmd after pfnPostRunCmd" );
...
void CL_PredictMovement( qboolean repredicting )
{
	...
	PS3_DIAG_FRAME( "pred: CL_PredictMovement enter, repredicting=%i", (int)repredicting );

	if( cls.demoplayback && !repredicting )
		CL_DemoInterpolateAngles();

	CL_SetUpPlayerPrediction( false, false );
	PS3_DIAG_FRAME( "pred: after CL_SetUpPlayerPrediction" );
	...
	CL_PushPMStates();
	CL_SetSolidPlayers( cl.playernum );
	PS3_DIAG_FRAME( "pred: before predict loop, ack=%u out=%u", (unsigned int)cls.netchan.incoming_acknowledged, (unsigned int)cls.netchan.outgoing_sequence );
	...
	for( i = 1; i < CL_UPDATE_MASK && cls.netchan.incoming_acknowledged + i < cls.netchan.outgoing_sequence + stoppoint; i++ )
	{
		...
		PS3_DIAG_FRAME( "pred: before CL_RunUsercmd i=%u cmd=%u runfuncs=%i", i, current_command, (int)runfuncs );
		CL_RunUsercmd( from, to, &to_cmd->cmd, runfuncs, &time, current_command );
		PS3_DIAG_FRAME( "pred: after CL_RunUsercmd i=%u", i );
