...
void CL_PlayVideo_f( void )
{
	...
	case 3:	// sequenced cinematics used this
		// a movie we can't open must not stall the playlist: cls.state never
		// becomes ca_cinematic, so SCR_RunCinematic would never reach
		// SCR_NextMovie again and the remaining entries would be dropped
		if( !SCR_PlayCinematic( Cmd_Argv( 1 )))
			SCR_NextMovie();
		break;
	}
}
