...
#if XASH_PS3
		// Key_SetKeyDest deliberately does NOT enable text input for PS3
		// (CL_Connect_f reaches the same case during map load and popped the
		// OSK over the loading screen). Toggling the console by hand is the
		// one console-dest switch the user actually asked to type at, so
		// request it here instead. force=true because host.textmode may
		// already be set. K_Y_BUTTON re-summons the OSK for the next command.
...
static int Con_DrawDebugLines( void )
{
	...
	for( int i = 0; i < ARRAYSIZE( con.notify ); i++, notify++ )
	{
		int x, len;
...
#if !XASH_PS3
	// recent-print-line flash over the HUD -- on PS3 the console log is a
	// loading/debug aid only (see Con_DrawConsole's ca_connecting path) and
	// must never appear once gameplay starts, so this notify-log overlay is
	// skipped entirely on this port. The chat input line below is unrelated
	// and still needs to draw.
	if( host.allow_console && !Con_BackgroundMapActive( ))
	{
		double time = cl.time;
...
{
	for( int i = 0; i < ARRAYSIZE( con.chars ); i++ )
...
static void Test_RunConHistory( void )
{
	...
	for( int i = 0; i < ARRAYSIZE( strs1 ); i++ )
	{
	...
	for( int i = 0; i < ARRAYSIZE( strs2 ); i++ )
	{
	...
	for( int i = ARRAYSIZE( strs2 ) - 1; i >= 0; i-- )
	{
		TASSERT_STR( input.buffer, strs2[i] );
