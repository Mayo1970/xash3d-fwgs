...
// the rate the mixer actually paints at. Normal channels already resample
// against snd.format.speed (S_MixNormalChannelsToRoombuffer), so any backend
// that opens the device at something other than SOUND_DMA_SPEED -- PS3 runs
// at 48000 -- needs the raw-sample paths to use the same number, or streamed
// audio (movies, mp3 soundtrack) comes out transposed
#define SOUND_OUTPUT_SPEED ( snd.format.speed ? snd.format.speed : SOUND_DMA_SPEED )
