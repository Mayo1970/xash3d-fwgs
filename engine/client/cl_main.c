...
CVAR_DEFINE_AUTO( cl_lw, DEFAULT_CL_LW, DEFAULT_CL_LW_FLAGS, "enable client weapon predicting" );
...
static void CL_CreateCmd( void )
{
	...
	PS3_DIAG_FRAME( "cl: CreateCmd before CL_SetSolidEntities" );
	CL_SetSolidEntities();
	PS3_DIAG_FRAME( "cl: CreateCmd after CL_SetSolidEntities" );
	CL_PushPMStates();
	CL_SetSolidPlayers( cl.playernum );
	PS3_DIAG_FRAME( "cl: CreateCmd after CL_SetSolidPlayers" );
	...
	PS3_DIAG_FRAME( "cl: CreateCmd before Platform_PreCreateMove, active=%i", (int)active );
	Platform_PreCreateMove();
	PS3_DIAG_FRAME( "cl: CreateCmd before dllFuncs.CL_CreateMove" );
	clgame.dllFuncs.CL_CreateMove( host.frametime, cmd, active );
	PS3_DIAG_FRAME( "cl: CreateCmd after dllFuncs.CL_CreateMove" );
	IN_EngineAppendMove( host.frametime, cmd, active );
	PS3_DIAG_FRAME( "cl: CreateCmd after IN_EngineAppendMove" );

	CL_PopPMStates();
	PS3_DIAG_FRAME( "cl: CreateCmd after CL_PopPMStates" );
	...
	PS3_DIAG_FRAME( "cl: CreateCmd before CL_PredictMovement" );
	CL_PredictMovement( false );
	PS3_DIAG_FRAME( "cl: CreateCmd after CL_PredictMovement, exit" );
}
...
static void CL_SendCommand( void )
{
	...
	PS3_DIAG_FRAME( "cl: before CL_CreateCmd, cls.state=%i", (int)cls.state );
	CL_CreateCmd();
	PS3_DIAG_FRAME( "cl: after CL_CreateCmd" );

	// clc_move, userinfo etc
	CL_WritePacket();
	PS3_DIAG_FRAME( "cl: after CL_WritePacket" );
}
...
#if XASH_PS3
	PS3_ProbeMemory( "CL_PrecacheResources done" );
#endif
	return true;
}
...
void Host_ClientBegin( void )
{
	...
	PS3_DIAG_FRAME( "cl: before Cbuf_Execute" );
	Cbuf_Execute ();
	PS3_DIAG_FRAME( "cl: after Cbuf_Execute" );
	...
	CL_CheckClientState();
	PS3_DIAG_FRAME( "cl: after CL_CheckClientState, cls.state=%i signon=%i", (int)cls.state, cls.signon );

	// check if logo cvars changed and re-upload if needed
	CL_CheckLogoChanged();
	PS3_DIAG_FRAME( "cl: after CL_CheckLogoChanged" );

	// tell the client.dll about client data
	CL_UpdateClientData();
	PS3_DIAG_FRAME( "cl: after CL_UpdateClientData" );

	// if running the server locally, make intentions now
	if( SV_Active( )) CL_SendCommand ();
	PS3_DIAG_FRAME( "cl: after CL_SendCommand" );

	SteamBroker_Frame();
	PS3_DIAG_FRAME( "cl: Host_ClientBegin exit" );
}
...
void Host_ClientFrame( void )
{
	...
	PS3_DIAG_FRAME( "cl: before pfnFrame" );
	clgame.dllFuncs.pfnFrame( host.frametime );
	PS3_DIAG_FRAME( "cl: after pfnFrame" );
	...
	CL_ReadPackets ();
	PS3_DIAG_FRAME( "cl: after CL_ReadPackets" );

	// do prediction again in case we got
	// a new portion updates from server
	CL_RedoPrediction ();
	PS3_DIAG_FRAME( "cl: after CL_RedoPrediction" );
	...
	CL_EmitEntities ();
	PS3_DIAG_FRAME( "cl: after CL_EmitEntities" );
	...
	// update the screen
	PS3_DIAG_FRAME( "cl: before SCR_UpdateScreen" );
	{
		// Everything the renderer costs, INCLUDING the vsync flip wait that
		// GL_SwapBuffers blocks in -- ps3gl reports that wait separately, so
		// subtract it to get real render CPU. See public/ps3_diag.h.
		double ps3_t = PS3_PHASE_NOW();
		SCR_UpdateScreen ();
		PS3_PHASE_ADD( ps3_phase_render, ps3_t );
	}
	PS3_DIAG_FRAME( "cl: after SCR_UpdateScreen" );

	// update audio
	{
		double ps3_t = PS3_PHASE_NOW();
		SND_UpdateSound ();
		PS3_PHASE_ADD( ps3_phase_sound, ps3_t );
	}
	...
	// adjust client time
	CL_AdjustClock ();
	PS3_DIAG_FRAME( "cl: Host_ClientFrame exit" );
}
...
void CL_Init( void )
{
	...
	// no SCR_BootProgress here: drawing before CL_LoadProgs calls a NULL
	// clgame.dllFuncs pointer -> silent PPU freeze on PS3 (confirmed on hardware)
