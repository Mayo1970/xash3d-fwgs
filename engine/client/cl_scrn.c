...
void SCR_UpdateScreen( void )
{
	...
	PS3_DIAG_FRAME( "cl: SCR_UpdateScreen enter, cls.state=%i", (int)cls.state );

	if( !V_PreRender( )) return;
	PS3_DIAG_FRAME( "cl: after V_PreRender" );
	...
	case ca_active:
		Con_RunConsole ();
		PS3_DIAG_FRAME( "cl: before V_RenderView" );
		V_RenderView();
		PS3_DIAG_FRAME( "cl: after V_RenderView" );
		break;
	case ca_cinematic:
		SCR_DrawCinematic();
		break;
	...
	{
		PS3_DIAG_FRAME( "cl: before V_PostRender" );
		V_PostRender();
		PS3_DIAG_FRAME( "cl: after V_PostRender" );
	}
}
...
Draw a standalone progress frame from inside a blocking load loop.

The engine only refreshes the screen from Host_Frame, so a long synchronous
load leaves the display frozen on whatever was there before, with no evidence
the console hasn't hung. On PC these loads are short enough not to matter; on
PS3 the per-file LV2 open/read latency stretches them into seconds.

Usable only once CL_LoadProgs has run (see the guard below), which rules out
the boot window itself -- the pre-menu black screen is FS_Init,
PS3_VerifyGameAssets and VID_Init, all of which precede any usable renderer.
The real caller is the VOX word preload on the first map load.

This deliberately does NOT honour cls.disable_screen. That flag exists to stop
the *normal* per-frame path from drawing over a loading plaque, but here we
are the only thing drawing and the plaque is exactly what we want to replace
with real progress. Every caller is a known-blocking loop sitting between two
Host_Frames, so no other code owns a half-built frame we could corrupt.

Callers may fire this per item; the 100 ms throttle keeps the flip cost off
the thing we are trying to speed up. Pass cur < 0 to skip the bar, and
force = true to defeat the throttle for a final/initial frame.
================
*/
void SCR_BootProgress( const char *stage, int cur, int total, qboolean force )
{
	static double last_draw;
	const rgba_t  white = { 255, 255, 255, 255 };
	double        now;
	int           bar_x, bar_y, bar_w, bar_h;
	int           text_w = 0, text_h = 0; // CL_DrawStringLen leaves both untouched if the font is not valid yet
	char          msg[128];

	// SCR_Init (and with it the console font) runs inside R_Init, so anything
	// before VID_Init has no way to draw -- callers there are a no-op.
	if( !ref.initialized || host.status == HOST_SLEEP )
		return;

	// R_BeginFrame's last act is gEngfuncs.CL_ExtraUpdate(), which calls
	// clgame.dllFuncs.IN_Accumulate() unconditionally -- and clgame.dllFuncs is
	// only populated by CL_LoadProgs, at the very end of CL_Init. Calling this
	// before then jumps through a NULL pointer; on PS3 that is an unhandled PPU
	// exception, so the app freezes dead with nothing in the log (cost one
	// hardware round to find). The window between VID_Init and CL_LoadProgs
	// therefore cannot be drawn into at all, and this refuses rather than
	// letting a future caller rediscover it the same way.
	if( !clgame.dllFuncs.IN_Accumulate )
		return;

	// host.realtime only advances in Host_Frame, which by definition is not
	// running while a caller of this function blocks
	now = Platform_DoubleTime();
	if( !force && ( now - last_draw ) < 0.1 )
		return;
	last_draw = now;

	if( cur >= 0 && total > 0 )
		Q_snprintf( msg, sizeof( msg ), "%s  %i%%", stage, bound( 0, ( cur * 100 ) / total, 100 ));
	else Q_strncpy( msg, stage, sizeof( msg ));

	bar_w = refState.width / 2;
	bar_h = 8;
	bar_x = ( refState.width - bar_w ) / 2;
	bar_y = refState.height / 2;

	ref.dllFuncs.R_BeginFrame( false );
	ref.dllFuncs.R_Set2DMode( true );
	ref.dllFuncs.R_ClearScreen();

	Con_DrawStringLen( msg, &text_w, &text_h );
	Con_DrawString(( refState.width - text_w ) / 2, bar_y - text_h - 8, msg, white );

	if( cur >= 0 && total > 0 )
	{
		// frame, then fill -- FillRGBA needs no texture, so this works before
		// any game content has been registered
		ref.dllFuncs.FillRGBA( kRenderNormal, bar_x - 1, bar_y - 1, bar_w + 2, bar_h + 2, 96, 96, 96, 255 );
		ref.dllFuncs.FillRGBA( kRenderNormal, bar_x, bar_y, ( bar_w * bound( 0, cur, total )) / total, bar_h, 255, 168, 0, 255 );
	}

	ref.dllFuncs.R_EndFrame();

	// nothing else drains the sysutil queue while we block here, and XMB will
	// wedge lv2 waiting on an app that never answers -- same trap already
	// fixed for the pre-video asset-verify window in Host_InitCommon
