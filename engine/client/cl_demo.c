...
// .dem is shared across machines in the HL community (not just a same-
// session self-consistency format), so every multi-byte field must
// round-trip through these on read/write, same convention as
// img_wad.c's WAD/mip swap tables
le_struct_begin( demoheader_swap )
	le_struct_field( demoheader_t, id )
	le_struct_field( demoheader_t, dem_protocol )
	le_struct_field( demoheader_t, net_protocol )
	le_struct_field( demoheader_t, host_fps )
	le_struct_field( demoheader_t, directory_offset )
le_struct_end();

le_struct_begin( demoentry_swap )
	le_struct_field( demoentry_t, entrytype )
	le_struct_field( demoentry_t, playback_time )
	le_struct_field( demoentry_t, playback_frames )
	le_struct_field( demoentry_t, offset )
	le_struct_field( demoentry_t, length )
	le_struct_field( demoentry_t, flags )
le_struct_end();
...
void CL_WriteDemoUserCmd( int cmdnumber )
{
	...
	{
		int outgoing_sequence_wire = LittleLong( cls.netchan.outgoing_sequence );
		int cmdnumber_wire = LittleLong( cmdnumber );
		FS_Write( cls.demofile, &outgoing_sequence_wire, sizeof( int ));
		FS_Write( cls.demofile, &cmdnumber_wire, sizeof( int ));
	}
	...
	{
		word bytes_wire = LittleShort( bytes );
		FS_Write( cls.demofile, &bytes_wire, sizeof( word ));
	}
...
static void CL_WriteDemoSequence( file_t *file )
{
	...
	wire = LittleLong( cls.netchan.incoming_sequence );
	FS_Write( file, &wire, sizeof( int ));
	wire = LittleLong( cls.netchan.incoming_acknowledged );
	FS_Write( file, &wire, sizeof( int ));
	wire = LittleLong( cls.netchan.incoming_reliable_acknowledged );
	FS_Write( file, &wire, sizeof( int ));
	wire = LittleLong( cls.netchan.incoming_reliable_sequence );
	FS_Write( file, &wire, sizeof( int ));
	wire = LittleLong( cls.netchan.outgoing_sequence );
	FS_Write( file, &wire, sizeof( int ));
	wire = LittleLong( cls.netchan.reliable_sequence );
	FS_Write( file, &wire, sizeof( int ));
	wire = LittleLong( cls.netchan.last_reliable_sequence );
	FS_Write( file, &wire, sizeof( int ));
}
...
void CL_WriteDemoMessage( qboolean startup, int start, sizebuf_t *msg )
{
	...
	{
		int swlen_wire = LittleLong( swlen );
		FS_Write( file, &swlen_wire, sizeof( int ));
	}
...
void GAME_EXPORT CL_WriteDemoUserMessage( int size, byte *buffer )
{
	...
	// write the length out.
	{
		int size_wire = LittleLong( size );
		FS_Write( cls.demofile, &size_wire, sizeof( int ));
	}
...
static void CL_WriteDemoHeader( const char *name )
{
	...
	double maxfps = fps_override.value ? MAX_FPS_HARD : MAX_FPS_SOFT;
	...
	// write header (via a scratch copy -- demo.header stays host-form for
	// the rest of the recording session, e.g. CL_GetDemoFramerate reads
	// demo.header.host_fps continuously)
	{
		demoheader_t hdr_wire = demo.header;
		le_struct_swap( demoheader_swap, &hdr_wire );
		FS_Write( cls.demofile, &hdr_wire, sizeof( hdr_wire ));
	}
...
static void CL_StopRecord( void )
{
	...
	float stoptime = CL_GetDemoRecordClock();
	if( clgame.hInstance ) clgame.dllFuncs.pfnReset();
	...
	{
		int numentries_wire = LittleLong( demo.directory.numentries );
		FS_Write( cls.demofile, &numentries_wire, sizeof( int ));
	}

	for( int i = 0; i < demo.directory.numentries; i++ )
	{
		demoentry_t entry_wire = demo.directory.entries[i];
		le_struct_swap( demoentry_swap, &entry_wire );
		FS_Write( cls.demofile, &entry_wire, sizeof( entry_wire ));
	}
	...
	int frames = cls.td_lastframe - cls.td_startframe;
...
static void CL_ReadDemoUserCmd( qboolean discard )
{
	...
	outgoing_sequence = LittleLong( outgoing_sequence );
	FS_Read( cls.demofile, &cmdnumber, sizeof( int ));
	cmdnumber = LittleLong( cmdnumber );
	FS_Read( cls.demofile, &bytes, sizeof( short ));
	bytes = LittleShort( bytes );
...
static void CL_ReadDemoSequence( qboolean discard )
{
	...
	incoming_sequence = LittleLong( incoming_sequence );
	FS_Read( cls.demofile, &incoming_acknowledged, sizeof( int ));
	incoming_acknowledged = LittleLong( incoming_acknowledged );
	FS_Read( cls.demofile, &incoming_reliable_acknowledged, sizeof( int ));
	incoming_reliable_acknowledged = LittleLong( incoming_reliable_acknowledged );
	FS_Read( cls.demofile, &incoming_reliable_sequence, sizeof( int ));
	incoming_reliable_sequence = LittleLong( incoming_reliable_sequence );
	FS_Read( cls.demofile, &outgoing_sequence, sizeof( int ));
	outgoing_sequence = LittleLong( outgoing_sequence );
	FS_Read( cls.demofile, &reliable_sequence, sizeof( int ));
	reliable_sequence = LittleLong( reliable_sequence );
	FS_Read( cls.demofile, &last_reliable_sequence, sizeof( int ));
	last_reliable_sequence = LittleLong( last_reliable_sequence );
...
static qboolean CL_DemoReadMessageQuake( byte *buffer, size_t *length )
{
	...
	*length = 0; // assume we fail
	...
	viewangles[0] = LittleFloat( viewangles[0] );
	FS_Read( cls.demofile, &viewangles[1], sizeof( float ));
	viewangles[1] = LittleFloat( viewangles[1] );
	FS_Read( cls.demofile, &viewangles[2], sizeof( float ));
	viewangles[2] = LittleFloat( viewangles[2] );
	...
	if( demo.lasttime != demo.timestamp )
	{
		// select entry into circular buffer
		demo.angle_position = (demo.angle_position + 1) & ANGLE_MASK;
		demoangle_t *a = &demo.cmds[demo.angle_position];
...
qboolean CL_DemoReadMessage( byte *buffer, size_t *length )
{
	...
	do
	{
		...
		case dem_userdata:
			{
				// on-disk field is a 32-bit int -- must not FS_Read()
				// directly into `size` (size_t, 8 bytes on this LP64
				// target): on big-endian that would land the 4 wire
				// bytes in the HIGH 32 bits of the 64-bit variable
				// instead of the low ones, producing a huge garbage
				// size regardless of the byte-swap below
				int size32 = 0;
				FS_Read( cls.demofile, &size32, sizeof( int ));
				size = (size_t)LittleLong( size32 );
			}
...
static void CL_DemoFindInterpolatedViewAngles( float t, float *frac, demoangle_t **prev, demoangle_t **next )
{
	...
	if( demo.cmds[i0].starttime >= t )
	{
		...
		{
			float at = demo.cmds[imod & ANGLE_MASK].starttime;
			if( at == 0.0f ) break;
...
int GAME_EXPORT CL_GetDemoComment( const char *demoname, char *comment )
{
	...
	le_struct_swap( demoheader_swap, &demohdr );
	...
	directory.numentries = LittleLong( directory.numentries );
	...
	for( int i = 0; i < directory.numentries; i++ )
	{
		...
		le_struct_swap( demoentry_swap, &entry );
...
static qboolean CL_ParseDemoHeader( const char *callee, const char *filename, file_t *f, demoheader_t *hdr, int32_t *numentries )
{
	...
	le_struct_swap( demoheader_swap, hdr );
	if( hdr->id != IDEMOHEADER )
	{
		Con_Printf( S_ERROR "%s: %s is not in supported format or not a demo file\n", callee, filename );
		return false;
	}
	...
	*numentries = LittleLong( *numentries );
