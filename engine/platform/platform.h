...
#include <errno.h>
#if XASH_PS3
#include <unistd.h> // usleep, for Platform_NanoSleep -- this libc has no nanosleep at all (no symbol, no declaration)
#endif
...
#elif XASH_PS3
	// PSL1GHT's libc has no nanosleep at all (not declared, no symbol in
	// any ppu lib) -- usleep is the finest-grained sleep actually available.
	usleep( nsec / 1000 );
	return true;
#elif XASH_POSIX
	struct timespec ts = {
...
#if XASH_SDL >= 2 || XASH_PSVITA || XASH_DOS || XASH_PS3 || XASH_USE_EVDEV
