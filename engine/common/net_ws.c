...
static qboolean NET_GetLong( byte *pData, size_t size, size_t *outSize, size_t splitsize, connprotocol_t proto )
{
	...
	if( proto == PROTO_GOLDSRC )
	{
		...
		sequence_number = LittleLong( pHeader->sequence_number );
		packet_id = pHeader->packet_id; // single byte, no swap needed
	...
	else
	{
		...
		sequence_number = LittleLong( pHeader->sequence_number );
		packet_id = LittleShort( pHeader->packet_id );
	...
	if( !NET_IsSocketError( ret ))
	{
		if( ret < NET_MAX_FRAGMENT )
		{
			...
			*length = ret;
			...
			if( sock == NS_CLIENT && LittleLong( *(int *)data ) == NET_HEADER_SPLITPACKET )
			{
...
static int NET_SendLong( netsrc_t sock, int net_socket, const char *buf, size_t len, int flags, const struct sockaddr_storage *to, size_t tolen, size_t splitsize )
{
	...
	if( splitsize > sizeof( SPLITPACKET ) && sock == NS_SERVER && len > splitsize )
	{
		...
		pPacket->sequence_number = LittleLong( net.sequence_number );
		pPacket->net_id = LittleLong( NET_HEADER_SPLITPACKET );
		int packet_number = 0;
		int total_sent = 0;
		...
		while( len > 0 )
		{
			...
			pPacket->packet_id = LittleShort((packet_number << 8) + packet_count);
