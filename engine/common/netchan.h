...
#if XASH_PS3
	// halved: these delta-history rings were 26.5Mb combined on hardware
	// (measured at the CS OOM death). Shortens delta lookback only, not
	// snapshot capacity (MAX_VISIBLE_PACKET) -- both sides fall back to a
	// full update gracefully. MULTIPLAYER_BACKUP left alone (remote-server protocol).
	#undef NUM_PACKET_ENTITIES
