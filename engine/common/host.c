...
#if !XASH_PS3
// Sink for the PS3 delta counters referenced from shared engine code; the
// ps3_diag.h stubs alias every one of them here so non-PS3 builds link.
unsigned int ps3_diag_dummy_counter = 0;
#endif
...
{
	// reference point for the PS3 stack watermark -- everything measured
	// below is a delta from this frame's stack depth
	PS3_DIAG_STACK_TOP();
	...
	PS3_DIAG_FRAME( "host: Host_Frame enter" );
	{
		// Coarse CPU phase split -- see public/ps3_diag.h. On non-PS3 builds
		// every PS3_PHASE_* below compiles to nothing.
		double ps3_t = PS3_PHASE_NOW();
		Host_InputFrame ();  // input frame
		PS3_PHASE_ADD( ps3_phase_input, ps3_t );
	}
	PS3_DIAG_FRAME( "host: after Host_InputFrame" );
	Host_ClientBegin (); // begin client
	PS3_DIAG_FRAME( "host: after Host_ClientBegin" );
	Host_GetCommands (); // dedicated in
	PS3_DIAG_FRAME( "host: after Host_GetCommands" );
	{
		// Server frame is the whole game DLL in single player: AI, physics,
		// entity think. Invisible to every renderer-side measurement so far.
		double ps3_t = PS3_PHASE_NOW();
		Host_ServerFrame (); // server frame
		PS3_PHASE_ADD( ps3_phase_server, ps3_t );
	}
	{
		// Host_ClientFrame subtracts its own render/sound sub-phases (timed in
		// cl_main.c), so ps3_phase_client ends up meaning "client frame minus
		// rendering minus sound" rather than double-counting them.
		double ps3_t = PS3_PHASE_NOW();
		Host_ClientFrame (); // client frame
		PS3_PHASE_ADD( ps3_phase_client, ps3_t );
	}
	PS3_DIAG_FRAME( "host: before HTTP_Run" );
	HTTP_Run();			 // both server and client
	PS3_DIAG_FRAME( "host: after HTTP_Run" );
	XRcon_Frame();
	PS3_DIAG_FRAME( "host: after XRcon_Frame, Host_Frame exit" );
...
#if XASH_PS3
	{
		extern void ps3gl_report_frame_phases( double input, double server, double client,
			double render, double sound, double total );
		extern void ps3gl_report_server_phases( double think, double link,
			unsigned int think_calls, unsigned int link_calls );
		extern void ps3gl_report_server_split( double readpackets, double gameframe,
			double sendmsgs );
		extern void ps3gl_report_snapshot_split( double addents, double emit,
			unsigned int addents_calls, unsigned int fullpack_calls );
		extern void ps3gl_report_delta_fields( unsigned int visited, unsigned int written,
			unsigned int entities, unsigned int with_encoder );
		extern void ps3gl_report_baseline( unsigned int calls, unsigned int tests,
			unsigned int iters, unsigned int newents, unsigned int num_instanced );

		ps3gl_report_frame_phases( ps3_phase_input, ps3_phase_server,
			ps3_phase_client - ps3_phase_render - ps3_phase_sound,
			ps3_phase_render, ps3_phase_sound, host.pureframetime );

		ps3gl_report_server_phases( ps3_phase_sv_think, ps3_phase_sv_link,
			ps3_phase_sv_think_calls, ps3_phase_sv_link_calls );

		ps3gl_report_server_split( ps3_phase_sv_readpackets, ps3_phase_sv_gameframe,
			ps3_phase_sv_sendmsgs );

		ps3gl_report_snapshot_split( ps3_phase_sv_addents, ps3_phase_sv_emit,
			ps3_phase_sv_addents_calls, ps3_phase_sv_fullpack_calls );

		ps3gl_report_delta_fields( ps3_delta_fields_visited, ps3_delta_fields_written,
			ps3_delta_entities, ps3_delta_with_encoder );

		ps3gl_report_baseline( ps3_baseline_calls, ps3_baseline_tests,
			ps3_baseline_iters, ps3_baseline_newents, ps3_baseline_num_instanced );

		ps3_phase_input = ps3_phase_server = ps3_phase_client = 0.0;
		ps3_phase_render = ps3_phase_sound = 0.0;
		ps3_phase_sv_think = ps3_phase_sv_link = 0.0;
		ps3_phase_sv_think_calls = ps3_phase_sv_link_calls = 0;
		ps3_phase_sv_readpackets = ps3_phase_sv_gameframe = 0.0;
		ps3_phase_sv_sendmsgs = 0.0;
		ps3_phase_sv_addents = ps3_phase_sv_emit = 0.0;
		ps3_phase_sv_addents_calls = ps3_phase_sv_fullpack_calls = 0;
		ps3_delta_fields_visited = ps3_delta_fields_written = 0;
		ps3_delta_entities = ps3_delta_with_encoder = 0;
		ps3_baseline_calls = ps3_baseline_tests = 0;
		ps3_baseline_iters = ps3_baseline_newents = 0;
		// ps3_baseline_num_instanced is a state snapshot, not a per-frame
		// count -- leave it so it keeps its last known value on frames where
		// no new entity appeared to re-read it
	}
#endif
}
...
#if XASH_PS3
	PS3_VerifyGameAssets();

	// Nothing drains the sysutil event queue between PS3_Init (boot) and the
	// first Host_Frame -- Platform_RunEvents (where XMB exit-game polling
	// normally happens) is a per-frame hook that hasn't run yet. If XMB posts
	// an event during this load window, lv2 waits on an app that never checks
	// for it -- the exact freeze class already fixed for quit (missing
	// SYSUTIL_EXIT_GAME drain), just relocated to boot. Asset checksums are
	// real file I/O (CRC32_File), long enough for this to matter.
