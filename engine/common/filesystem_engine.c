...
static qboolean FS_DetermineRootDirectory( char *out, size_t size )
{
	...
	return true;
#elif XASH_PS3
	// getcwd() on PS3 returns "/" (LV2 filesystem root, not the app's own
	// USRDIR -- the homebrew loader does not chdir into it before exec), and
	// COM_StripDirectorySlash() below then collapses that single "/" into an
	// empty string, which every downstream FS lookup silently resolves
	// against -- confirmed on real hardware, not guessed (see AGENTS.md
	// goal-stack #3). Sibling PS4 port hits the identical "no real cwd on a
	// console" problem and hardcodes its own known install path the same
	// way (ioQuake3-PS4's Sys_Cwd() -> "/app0"); do the same here, but point
	// at a fixed external data dir (like Quake3-PS3's ps3_basepath) instead
	// of the PKG's own USRDIR, so valve/ isn't duplicated per install.
	Q_strncpy( out, "/dev_hdd0/data/xash3dfwgs", size );
	return true;
#elif XASH_PSVITA
