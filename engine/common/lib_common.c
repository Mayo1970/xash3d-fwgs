...
const char *COM_OffsetNameForFunction( void *function )
{
	...
	// NOT %zu: the newlib linked on this port has no C99 %z/%j/%t length
	// modifiers (jump table in _svfprintf_r tops out at 'x', 'z' falls to
	// the default case and prints literally as "z"). "ofs:%zu" therefore
	// always rendered as the literal string "ofs:zu" on hardware, which
	// COM_FunctionFromName_SR's Q_atoi() parses back as offset 0 -- every
	// restored FIELD_FUNCTION pointer silently resolved to
	// svgame.dllFuncs.pfnGameInit itself (offset+0), so any transferred
	// entity with a saved think/use/touch/blocked function pointer had it
	// replaced with GameDLLInit. %lu is unambiguous across every libc.
