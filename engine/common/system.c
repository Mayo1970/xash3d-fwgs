...
#if XASH_PS3
// PSL1GHT's sysutil/sysutil.h declares sysUtilCheckCallback() as foo() instead
// of foo(void), same -Werror=strict-prototypes issue as sys_ps3.c works around.
...
const char *Sys_GetCurrentUser( void )
{
	...
	// each sysUtil param needs its OWN exact buffer size or it returns INVALID_VALUE
	static char username[SYSUTIL_SYSTEMPARAM_CURRENT_USERNAME_SIZE];
	...
	if( COM_StringEmpty( username ))
	{
		static char nickname[SYSUTIL_SYSTEMPARAM_NICKNAME_SIZE];

		nickname[0] = '\0';
		if( sysUtilGetSystemParamString( SYSUTIL_SYSTEMPARAM_ID_NICKNAME,
			nickname, SYSUTIL_SYSTEMPARAM_NICKNAME_SIZE ) == 0 && !COM_StringEmpty( nickname ))
			return nickname;
	}
...
{
#if !XASH_DEDICATED
	// Previously skipped on PS3 because Con_Print() fed Con_DrawNotify's
	// per-frame on-screen overlay, which fought with in-game HUD text and
	// burned render time for output nobody reads off the TV. That overlay
	// is now compiled out during gameplay on this port (see the XASH_PS3
	// guard in Con_DrawNotify, engine/client/console.c), so buffering here
	// is just cheap string appends again -- and it's what supplies the
	// loading-screen/manual console (Con_DrawSolidConsole) with any text at
	// all. The UDP sink below (Sys_PrintLog -> PS3_Printf) remains the
	// primary debug channel, see public/ps3_diag.h.
	if( !Host_IsDedicated() )
	{
		Con_Print( pMsg );
	}
...
#elif XASH_PS3
	// PSL1GHT has no execv() (or any exec-family call) at all, and
	// wai_getExecutablePath() always returns -1 here (see whereami.c) so
	// Sys_CanRestart() already reports false -- this is unreachable in
	// practice, but the code still needs to compile.
	printf( "Sys_NewInstance: not supported on PS3\n" );
#else
	int exelen = wai_getExecutablePath( NULL, 0, NULL );
	if( exelen >= 0 )
	{
