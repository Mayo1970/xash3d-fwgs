...
// .hpk is a cross-machine format ("custom user package to send other
// clients", see hpak.h) -- every multi-byte field must round-trip through
// these on read/write, same convention as img_wad.c's WAD/mip swap tables.
le_struct_begin( dresource_swap )
	le_struct_field( dresource_t, type )
	le_struct_field( dresource_t, nIndex )
	le_struct_field( dresource_t, nDownloadSize )
	le_struct_field( dresource_t, pNext )
	le_struct_field( dresource_t, pPrev )
le_struct_end();

le_struct_begin( hpak_header_swap )
	le_struct_field( hpak_header_t, ident )
	le_struct_field( hpak_header_t, version )
	le_struct_field( hpak_header_t, infotableofs )
le_struct_end();

le_struct_begin( hpak_lump_swap )
	le_struct_child( hpak_lump_t, resource, dresource_swap )
	le_struct_field( hpak_lump_t, filepos )
	le_struct_field( hpak_lump_t, disksize )
le_struct_end();

// self-inverse: also used to convert a freshly read array back to host form,
// or a host-form array to wire form right before writing it out
static void HPAK_SwapLumps( hpak_lump_t *entries, int count )
{
	int i;

	for( i = 0; i < count; i++ )
		le_struct_swap( hpak_lump_swap, &entries[i] );
}
...
static void HPAK_CreatePak( const char *filename, resource_t *pResource, byte *pData, file_t *fin )
{
	...
	{
		hpak_header_t hdr_wire = hash_pack_header;
		le_struct_swap( hpak_header_swap, &hdr_wire );
		FS_Write( fout, &hdr_wire, sizeof( hdr_wire ));
	}
	...
	{
		int count_wire = LittleLong( hash_pack_info.count );
		FS_Write( fout, &count_wire, sizeof( count_wire ));
	}
	{
		hpak_lump_t lump_wire = hash_pack_info.entries[0];
		le_struct_swap( hpak_lump_swap, &lump_wire );
		FS_Write( fout, &lump_wire, sizeof( lump_wire ));
	}
...
void HPAK_AddLump( qboolean bUseQueue, const char *name, resource_t *pResource, byte *pData, file_t *pFile )
{
	...
	le_struct_swap( hpak_header_swap, &hash_pack_header );
	...
	srcpak.count = LittleLong( srcpak.count );
	...
	HPAK_SwapLumps( srcpak.entries, srcpak.count );
	...
	{
		int count_wire = LittleLong( dstpak.count );
		FS_Write( file_dst, &count_wire, sizeof( count_wire ));
	}

	HPAK_SwapLumps( dstpak.entries, dstpak.count );
	...
	FS_Seek( file_dst, 0, SEEK_SET );
	{
		hpak_header_t hdr_wire = hash_pack_header;
		le_struct_swap( hpak_header_swap, &hdr_wire );
		FS_Write( file_dst, &hdr_wire, sizeof( hdr_wire ));
	}
...
static qboolean HPAK_Validate( const char *filename, qboolean quiet, qboolean delete )
{
	...
	le_struct_swap( hpak_header_swap, &hdr );
	...
	num_lumps = LittleLong( num_lumps );
	...
	HPAK_SwapLumps( dataDir, num_lumps );
...
qboolean HPAK_ResourceForHash( const char *filename, byte *hash, resource_t *pResource )
{
	...
	le_struct_swap( hpak_header_swap, &header );
	...
	directory.count = LittleLong( directory.count );
	...
	HPAK_SwapLumps( directory.entries, directory.count );
...
void HPAK_RemoveLump( const char *name, resource_t *pResource )
{
	...
	// header copy -- write the raw wire-order bytes through untouched first,
	// only THEN swap our own in-memory copy to host form for our own use
	// below (swapping before this write would corrupt the passthrough copy)
	FS_Read( file_src, &hash_pack_header, sizeof( hpak_header_t ));
	...
	hpak_read.count = LittleLong( hpak_read.count );
	...
	HPAK_SwapLumps( hpak_read.entries, hpak_read.count );
	...
	hash_pack_header.infotableofs = FS_Tell( file_dst );
	{
		int count_wire = LittleLong( hpak_save.count );
		FS_Write( file_dst, &count_wire, sizeof( count_wire ));
	}

	HPAK_SwapLumps( hpak_save.entries, hpak_save.count );
