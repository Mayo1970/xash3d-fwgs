...
void Netchan_TransmitBits( netchan_t *chan, int length, const byte *data )
{
	...
	PS3_DIAG_FRAME( "net: Netchan_TransmitBits enter, length=%i", length );
	...
	if( !CL_IsPlaybackDemo( ))
	{
		...
		PS3_DIAG_FRAME( "net: before NET_SendPacketEx, bytes=%i addrtype=%i",
			MSG_GetNumBytesWritten( &send ), (int)chan->remote_address.type );
		NET_SendPacketEx( chan->sock, MSG_GetNumBytesWritten( &send ), MSG_GetData( &send ), chan->remote_address, splitsize );
		PS3_DIAG_FRAME( "net: after NET_SendPacketEx" );
	}
