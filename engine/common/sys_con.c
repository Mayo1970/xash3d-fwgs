...
#if XASH_PS3
	// no GDB stub, no attached terminal -- the UDP debug log sink set up in
	// PS3_Init is the only observability channel until video bring-up
	PS3_Printf( "%s%s", logtime, stripped );
#endif

#if !XASH_MOBILE_PLATFORM && !XASH_WIN32 && !XASH_PS3 // Wcon does the job; PS3 has no attached terminal, UDP sink above is the only sink
