...
void *R_StudioGetAnim( studiohdr_t *m_pStudioHeader, model_t *m_pSubModel, mstudioseqdesc_t *pseqdesc )
{
	...
	if( !Mod_CacheCheck(( cache_user_t *)&( paSequences[pseqdesc->seqgroup] )))
	{
		...
		// the group file is cached as a whole (Mod_CacheCheck above gates this
		// entire block to run once), so every sequence living in it must be
		// swapped now, not just pseqdesc -- any other sequence sharing this
		// seqgroup would otherwise be returned little-endian on every later
		// call, which R_StudioCalcBones then walks with garbage offset[]
		// values (a silent out-of-bounds pointer walk, not a catchable fault
		// on hardware with no MMU)
		int swapped = 0;

		for( int i = 0; i < m_pStudioHeader->numseq; i++ )
		{
			mstudioseqdesc_t *pseq = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + i;

			if( pseq->seqgroup != pseqdesc->seqgroup )
				continue;

			Mod_SwapStudioSeqGroupAnims( m_pStudioHeader, pseq,
				(byte *)paSequences[pseqdesc->seqgroup].data );
			swapped++;
		}

#if XASH_PS3
		// unconditional and load-event-tied (not per-frame), so it needs no
		// PS3_DIAG arming and fires on any map -- confirms this fixed code
		// path actually ran, independent of where in the game a group file
		// happens to get demand-loaded
		PS3_Printf( "mdl: swapped %d/%d seqs for group %s (seqgroup=%d)\n",
			swapped, m_pStudioHeader->numseq, filepath, pseqdesc->seqgroup );
#endif
	}
...
====================
*/
// defined below; validates and byteswaps the loaded studio buffer
...
qboolean Mod_GetStudioBounds( const char *name, vec3_t mins, vec3_t maxs )
{
	...
	f = FS_LoadFile( name, &filesize, false );
	if( !f ) return false;

	// raw LE magic compare never matched on PS3; use the loader's own
	// validate+byteswap pass instead (no-op swap on little-endian)
	if( Mod_SwapStudioModel( name, f, filesize ))
	{
		VectorClear( mins );
		VectorClear( maxs );
