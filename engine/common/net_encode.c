...
static void Delta_ClearTables( qboolean keep_encoders );
...
/*
=====================
Delta_CheckFieldWidth

delta.lst is game data and is free to describe a field with a wire type that is
not the width of the C member it points at. Stock Half-Life does it twice:
entity_state_t::eflags is a byte described as DT_INTEGER, and
custom_entity_state_t::skin is a short described as DT_INTEGER.

The codec keys off pField->size (see Delta_LoadIntegerField), so both byte orders
handle these correctly. This is a report only -- on big-endian a mismatch used to
silently pick up the wrong half of the member, so it is worth surfacing when a
mod or a different game ships its own delta.lst.
=====================
*/
static void Delta_CheckFieldWidth( const delta_info_t *dt, const delta_t *pField )
{
	int declared;

	// mirror the flag precedence the codec itself uses
	if( FBitSet( pField->flags, DT_BYTE ))
		...
		declared = 4;
	else if( FBitSet( pField->flags, DT_ANGLE|DT_FLOAT|DT_TIMEWINDOW_8|DT_TIMEWINDOW_BIG ))
		declared = 4;
	else return; // DT_STRING is compared and copied as a string, width is irrelevant

	if( declared != pField->size )
	{
		Con_DPrintf( S_WARN "%s: %s->%s is %d bytes, delta description declares a %d-byte type\n",
			__func__, dt->pName, pField->name, pField->size, declared );
	}
}
...
static qboolean Delta_AddField( delta_info_t *dt, const char *pName, int flags, int bits, float mul, float post_mul )
{
	...
	for( i = 0, pField = dt->pFields; i < dt->numFields && pField; i++, pField++ )
	{
		if( !Q_strcmp( pField->name, pName ))
		{
			...
			Delta_CheckFieldWidth( dt, pField );
			return true;
		}
	}
...
void Delta_ParseTableField( sizebuf_t *msg )
{
	...
	// rebuild the field lists from the wire, but keep the game .dll's encoders
	if( delta_init )
		Delta_ClearTables( true );
...
{
	// re-parse delta.lst from scratch, but the game .dll is still loaded and
	// will not be asked to register its encoders again -- keep them
	if( delta_init ) Delta_ClearTables( true );
...
void Delta_InitClient( void )
{
	...
	for( int i = 0; i < ARRAYSIZE( dt_info ); i++ )
	{
		...
		if( dt_info[i].userCallback )
			numEncoders++;
	}

	if( numActive ) delta_init = true;

	// this runs after the last point that rebuilds the tables (the svc_deltatable
	// burst), so it is where the game .dll's encoders can be seen to have survived
	// -- see Delta_ClearTables. Expect 3 on a listen server: entity_state_t,
	// entity_state_player_t and custom_entity_state_t. 0 means they were wiped
	// again and every conditionally-suppressed field is going out unconditionally.
	Con_DPrintf( "Delta: %d tables active, %d game encoders registered\n", numActive, numEncoders );
}
...
Drop the parsed field lists so they can be rebuilt.

keep_encoders separates the two reasons this happens. The tables are rebuilt
several times while the game .dll stays loaded: delta.lst is re-parsed on every
map spawn (SV_SpawnServer), and they are rebuilt again from the wire when a
client -- including the local client of a listen server -- parses the
svc_deltatable burst in SV_SendServerdata. The encoder callback, though, belongs
to the loaded game .dll, and the .dll is only asked to register it once, from
SV_LoadProgs, which early-returns while svgame.hInstance is live. Clearing the
callbacks on a rebuild therefore left Entity_Encode/Player_Encode/Custom_Encode
NULL for the whole session -- Delta_CustomEncode then activates every field, so
conditionally-suppressed fields were transmitted unconditionally.

The full teardown (keep_encoders == false) is what Delta_Shutdown does, and is
correct only where the game .dll itself is going away: those function pointers
dangle once it unloads.
=====================
*/
static void Delta_ClearTables( qboolean keep_encoders )
{
...
/*
=====================
Delta_LoadIntegerField / Delta_StoreIntegerField

Access an integer field at the width of the C member it actually points at,
which is not necessarily the width implied by its DT_BYTE/DT_SHORT/DT_INTEGER
wire type. delta.lst is free to describe a field with a different type than the
struct declares -- entity_state_t::skin, for instance, is a `short` described as
DT_INTEGER, and clientdata_t/weapon_data_t have similar mismatches.

Reading such a field at the wire width picks up the wrong half of the member on
a big-endian target: a 4-byte load over `short skin; short solid;` yields
(skin << 16) | solid, and the subsequent clamp to the field's bit count keeps
`solid`. On little-endian the wanted bits happen to land in the low half, so the
mismatch is invisible and the value round-trips by accident. Writing is worse --
a 4-byte store scribbles over the neighbouring member.

pField->size is sizeof() of the real member (see the *_DEF macros), so keying off
it is correct on both byte orders and reproduces existing little-endian results.
=====================
*/
static uint Delta_LoadIntegerField( const delta_t *pField, const void *base, int signbit )
{
	const byte *p = (const byte *)base + pField->offset;

	switch( pField->size )
	{
	case 1:
		return signbit ? (uint)(int)*(const int8_t *)p : (uint)*(const uint8_t *)p;
	case 2:
		return signbit ? (uint)(int)*(const int16_t *)p : (uint)*(const uint16_t *)p;
	default:
		return signbit ? (uint)*(const int32_t *)p : *(const uint32_t *)p;
	}
}

static void Delta_StoreIntegerField( const delta_t *pField, void *base, uint iValue, int signbit )
{
	byte *p = (byte *)base + pField->offset;

	switch( pField->size )
	{
	case 1:
		if( signbit ) *(int8_t *)p = (int8_t)iValue;
		else *(uint8_t *)p = (uint8_t)iValue;
		break;
	case 2:
		if( signbit ) *(int16_t *)p = (int16_t)iValue;
		else *(uint16_t *)p = (uint16_t)iValue;
		break;
	default:
		if( signbit ) *(int32_t *)p = (int32_t)iValue;
		else *(uint32_t *)p = iValue;
		break;
	}
}
...
static qboolean Delta_CompareField( delta_t *pField, const void *from, const void *to )
{
	...
	if( pField->flags & ( DT_BYTE|DT_SHORT|DT_INTEGER ))
	{
		fromF = Delta_LoadIntegerField( pField, from, signbit );
		toF = Delta_LoadIntegerField( pField, to, signbit );
...
static void Delta_WriteField_( sizebuf_t *msg, delta_t *pField, const void *from, const void *to, double timebase )
{
	...
	if( pField->flags & ( DT_BYTE|DT_SHORT|DT_INTEGER ))
	{
		iValue = Delta_LoadIntegerField( pField, to, signbit );
...
static void Delta_CopyField( delta_t *pField, const void *from, void *to, double timebase )
{
	...
	if( FBitSet( pField->flags, DT_BYTE|DT_SHORT|DT_INTEGER ))
	{
		Delta_StoreIntegerField( pField, to,
			Delta_LoadIntegerField( pField, from, bSigned ), bSigned );
	}
...
static void Delta_ReadField_( sizebuf_t *msg, delta_t *pField, void *to, double timebase )
{
	...
	if( pField->flags & ( DT_BYTE|DT_SHORT|DT_INTEGER ))
	{
		...
		Delta_StoreIntegerField( pField, to, iValue, bSigned );
	}
	else if( pField->flags & DT_FLOAT )
	{
		iValue = MSG_ReadBitLong( msg, pField->bits, bSigned );
...
void MSG_WriteDeltaEntity( const entity_state_t *from, const entity_state_t *to, sizebuf_t *msg, qboolean force, int delta_type, double timebase, int baseline )
{
	...
	// Telemetry: fields visited vs fields actually written, plus whether this
	// struct had a live game-DLL encoder. If written/visited is ~1.0 with
	// encoders==0, the encoders were wiped and every conditionally-suppressed
	// field is going out unconditionally (see Delta_ClearTables) -- that is a
	// regression of a known bug, not a slow comparison scan.
	ps3_delta_fields_visited += dt->numFields;
	ps3_delta_fields_written += numChanges;
	ps3_delta_entities++;
	if( dt->userCallback )
		ps3_delta_with_encoder++;
