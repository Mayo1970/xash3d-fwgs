...
// dumps the heap by pool right before Sys_Error ends the process (no debugger
// on PS3). Guarded against re-entry since Con_Printf itself allocates.
static void Mem_ReportOutOfMemory( poolhandle_t poolptr, size_t size, const char *filename, int fileline )
{
	static qboolean in_oom_dump = false;
	...
	Con_Printf( S_ERROR "OUT OF MEMORY: %s requested for pool \"%s\" at %s:%i -- heap follows\n",
		Q_memprint( size ), Mem_PoolName( poolptr ), filename, fileline );
...
void *_Mem_Alloc( poolhandle_t poolptr, size_t size, qboolean clear, const char *filename, int fileline )
{
	...
	if( FBitSet( pool->flags, MEM_SMALL_ALLOC_OPT ) && size <= MEM_SMALL_MAX )
	{
		...
		if( mem == NULL )
		{
			Mem_ReportOutOfMemory( poolptr, size, filename, fileline );
			Sys_Error( "%s: out of memory (alloc size %s, pool \"%s\", at %s:%i)\n", __func__, Q_memprint( size ), Mem_PoolName( poolptr ), filename, fileline );
			return NULL;
		}
...
void *_Mem_Realloc( poolhandle_t poolptr, void *data, size_t size, qboolean clear, const char *filename, int fileline )
{
	...
	if( Mem_ReadSentinel( data ) == MEMHEADER_SENTINEL_SMALL )
	{
		...
		{
			Mem_ReportOutOfMemory( poolptr, size, filename, fileline );
			Sys_Error( "%s: out of memory (realloc size %s, old size %s, pool \"%s\", at %s:%i)\n", __func__, Q_memprint( size ), Q_memprint( oldsize ), Mem_PoolName( poolptr ), filename, fileline );
			return NULL;
		}
...
void Mem_PrintStats( void )
{
	...
	// %zu isn't implemented by this newlib's printf (renders literally); cast to %u
	Con_Printf( "^3%u^7 memory pools, totalling: ^1%s\n", (unsigned int)count, Q_memprint( size ));
...
// same report as the `memlist` command, callable from code
void Mem_DumpPools( size_t minallocationsize )
{
	Mem_PrintList( minallocationsize );
	Mem_PrintStats();
}
