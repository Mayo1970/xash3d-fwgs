...
{
#if XASH_PS3
	/* Goal 16: the `entities` scene phase measured 10.8ms of a 16.8ms frame
	 * with flip wait at 0.00ms (RSX idle, PPE-bound), and nothing measured
	 * inside it. It is NOT all studio models -- brush/sprite entities,
	 * CL_DrawEFX, the game DLL's two triangle callbacks and the viewmodel all
	 * run here too. These six buckets settle which one it is before anything
	 * is optimized.
	 *
	 * ps3gl_ticks() rather than Platform_DoubleTime(): the latter is
	 * gettimeofday(), an LV2 syscall at 1us granularity, and these seams fire
	 * per entity. Declared extern locally for the same reason as the
	 * scene-phase call in R_RenderScene below -- objcopy -G localizes ref_gl's
	 * data symbols, so only an undefined function call crosses out of here. */
	extern uint64_t ps3gl_ticks( void );
	extern void ps3gl_report_entity_phases( uint64_t studio, uint64_t brush,
		uint64_t sprite, uint64_t efx, uint64_t clienttri, uint64_t viewmodel );
	uint64_t ps3_studio = 0, ps3_brush = 0, ps3_sprite = 0;
	uint64_t ps3_efx = 0, ps3_clienttri = 0, ps3_viewmodel = 0;
	uint64_t ps3_t;
#define PS3_ENT_BEGIN()		( ps3_t = ps3gl_ticks() )
#define PS3_ENT_END( acc )	(( acc ) += ps3gl_ticks() - ps3_t )
#else
#define PS3_ENT_BEGIN()		((void)0)
#define PS3_ENT_END( acc )	((void)0)
#endif
	...
	for( int i = 0; i < tr.draw_list->num_solid_entities && !FBitSet( RI.rvp.flags, RF_ONLY_CLIENTDRAW ); i++ )
	{
		...
		case mod_brush:
			...
			PS3_ENT_END( ps3_brush );
			break;
		case mod_alias:
			PS3_ENT_BEGIN();
			R_DrawAliasModel( RI.currententity );
			PS3_ENT_END( ps3_brush );	// alias models are rare in HL1; folded into brush
			break;
		case mod_studio:
			PS3_ENT_BEGIN();
			R_DrawStudioModel( RI.currententity );
			PS3_ENT_END( ps3_studio );
			break;
		default:
			break;
		}
...
#if XASH_PS3
	ps3gl_report_entity_phases( ps3_studio, ps3_brush, ps3_sprite,
		ps3_efx, ps3_clienttri, ps3_viewmodel );
	/* Drains gl_studio.c's finer split of the studio + viewmodel buckets above.
	 * Called from here rather than from gl_studio.c itself because this is the
	 * one place that runs exactly once per scene pass, after every studio model
	 * (including the viewmodel) has been drawn. */
	R_StudioFlushPS3Phases();
#endif
#undef PS3_ENT_BEGIN
#undef PS3_ENT_END
}
...
void R_RenderScene( void )
{
...
#if XASH_PS3
	// Wall time of everything from here to R_DrawWorld -- dlight push, frustum
	// and frame setup, R_Clear, R_MarkLeaves, fog, ripple animation. Measured
	// as one bucket because it is expected to be small; if it is not, that is
	// itself the finding. See the report call at the bottom of this function.
	double ps3_scene_start;
	{
		extern double Platform_DoubleTime( void );
		ps3_scene_start = Platform_DoubleTime();
	}
#endif
	...
	R_CheckGLFog();
#if XASH_PS3
	/* Subdivide the render phase. Hardware round 2 (2026-07-29) put ~75% of
	 * the frame in SCR_UpdateScreen with the game DLL at only ~2ms, but no
	 * world-surface counter correlated cleanly with it -- and studio-model
	 * entity rendering (CPU bone transforms + per-vertex lighting) is
	 * invisible to every counter that existed. These four seams settle it.
	 *
	 * Reported by function call, not extern data: --static-linking's
	 * `objcopy -G lib_ref_gl_exports` localizes every data symbol in ref_gl
	 * except GetRefAPI, so only an undefined function call crosses out of
	 * here -- same mechanism as pglFoo() and ps3gl_report_dlight_cost. */
	{
		extern double Platform_DoubleTime( void );
		extern void ps3gl_report_scene_phases( double setup, double world,
			double entities, double water );
		double t = Platform_DoubleTime();
		double t_setup = t - ps3_scene_start;
		R_DrawWorld();
		R_CheckFog();
		double t_world = Platform_DoubleTime() - t;

		gEngfuncs.CL_ExtraUpdate ();	// don't let sound get messed up if going slow

		t = Platform_DoubleTime();
		R_DrawEntitiesOnList();
		double t_ents = Platform_DoubleTime() - t;

		t = Platform_DoubleTime();
		R_DrawWaterSurfaces();
		double t_water = Platform_DoubleTime() - t;

		/* studio_models used to be passed here as r_stats.c_studio_models_drawn,
		 * which GL_BackendEndFrame only zeroes when r_speeds > 0 -- so in a
		 * default build it printed a lifetime running total. The count now
		 * rides along with ps3gl_report_studio_phases from gl_studio.c. */
		ps3gl_report_scene_phases( t_setup, t_world, t_ents, t_water );
	}
#else
	R_DrawWorld();
	R_CheckFog();
