...
void R_DrawBrushModel( cl_entity_t *pent )
{
	...
	// second ~391KB stack frame, nested one level below R_DrawEntitiesOnList
	PS3_DIAG_STACK( "R_DrawBrushModel" );
...
static void R_EdgeDrawing( void )
{
	...
	// this frame carries ledges[]/lsurfs[] -- the largest stack allocation
	// anywhere in the engine, and the reason this measurement exists
	PS3_DIAG_STACK( "R_EdgeDrawing" );

	R_BeginEdgeFrame();

	// this will prepare edges
	PS3_DIAG( "ref: before R_RenderWorld" );
	R_RenderWorld();
	PS3_DIAG( "ref: after R_RenderWorld" );

	// move brushes to separate list to merge with edges?
	R_DrawBEntitiesOnList();
	PS3_DIAG( "ref: after R_DrawBEntitiesOnList" );

	// display all edges
	R_ScanEdges();
	PS3_DIAG( "ref: after R_ScanEdges" );
}
...
{
	PS3_DIAG( "ref: R_RenderScene enter" );
...
//	R_SetupGL( true );
	...
	PS3_DIAG( "ref: before R_MarkLeaves" );
	R_MarkLeaves();
	// R_PushDlights (r_worldmodel); ??
	// R_DrawWorld();
	PS3_DIAG( "ref: before R_EdgeDrawing" );
	R_EdgeDrawing();
	PS3_DIAG( "ref: after R_EdgeDrawing" );

	gEngfuncs.CL_ExtraUpdate(); // don't let sound get messed up if going slow

	PS3_DIAG( "ref: before R_DrawEntitiesOnList" );
	R_DrawEntitiesOnList();
	PS3_DIAG( "ref: after R_DrawEntitiesOnList" );
